...
/* Need an option to allow SSID's */

int callcheck_withssid( char *str )
{
  char *c, call[10];
  int retval;
  ...
  for( c = call; *c; c++ ){
	...
	/* Allow -1 and -10 through -15 */
	if((*(c+1) != '\0') && ((*(c+1) < '0') || (*(c+1) > '5')))
	  return 0;
      } else
	/* Allow -2 through -9 */
	if(*(c+1) != '\0')
  ...
  /* Now we've stripped off any SSID, check the call that's left. */
  retval = callcheck( call );
