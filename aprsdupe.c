...
/* 24Sep2023, WZ0C, increasing to 50 to account for 30 seconds
 * worth of APRS packets, plus whatever email packets we think
 * we'll receive in 5 minutes (per the duration in that code).
 */
#define	MAXDUPS	50
...
static int16 calc_aprs_crc (char *srccall, char *destcall, char *payload, int datalen)
{
	int16 crc, len;
	char tocall[10], *cp;
	 ...
	 * 
	 * Duplicate checking is based on the origin call and SSID, 
	 * destination call (SSID ignored), data length, and data 
	 * content. Note that the path is ignored in duplicate checking. 
	 * The origin and destination calls are case-sensitive. 
	 */

	strcpy( tocall, destcall );
	if( (cp = strchr( tocall, '-' )) != NULL )
	  *cp = '\0';

	aprslog( -1, "calc_aprs_crc: %s -> %s (%d bytes)", srccall, tocall, datalen );
	
	/* start computing the checksum */
	crc = 0xffff;

	/* srccall first */
	len = (int16) strlen (srccall);
	while (len--)
		crc = FCS(crc, *srccall++);

	/* then destcall */
	len = (int16) strlen (tocall);
	cp = tocall;
	while (len--)
		crc = FCS(crc, *cp++);
...
int aprs_dup (char *srccall, char *destcall, char *payload, int datalen, int age_seconds)
{
  int16 checksum = calc_aprs_crc (srccall, destcall, payload, datalen);
...
int doaprsdup (int argc, char *argv[], void *p OPTIONAL)
{
	...
	else if (!stricmp (argv[1], "test"))
	{
		...
		ret = aprs_digi (&hdr, idest, &nomark, "test", 4, addr );
