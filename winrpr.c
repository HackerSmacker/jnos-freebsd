/*
 * Support for both WINRPR and QTSoundModem software modems
 *
 ...
 *
 * January 14, 2023, by Maiko (VE4KLM) - QtSoundmodem (all 4 ports) !
 *  
 * This driver also works with QtSoundmodem - big thanks to Niels (PD9Q) for
 * noticing this last year, but he also noticed JNOS crashing on any attempt
 * to attach additional kiss ports. This is now fixed, and I have been able
 * to get all 4 modems working with the one driver, example :
 *
 *   attach winrpr QtK 192.168.100.105 8105
 *
 *   trace QtK 0x211 QtK.log 1000
 *
...
int winrpr_attach (int argc, char *argv[], void *p)
{
	...
	ifp->flags = 0;		/* 06May2023, Michael (WZ0C), moved before setencap
							or else port does not show in jheard */
	setencap (ifp, "AX25");

	ifp->ioctl = NULL;
	ifp->raw = winrpr_raw;
	ifp->show = NULL;
	ifp->port = 0;	/* 10Jan2023, Maiko, Oops, forgot this !!! */
	 ...
	 *
	 * 14Jan2023, Maiko (VE4KLM), no change for this, but just a note. We have
	 * to use the SLIP structures if we are to attach additional kiss ports to
	 * the original device. WinRPR did not support multiple devices the last I
	 * checked, but QtSoundModem supports up to 4 of them, all diff modes :]
	 */
	...
	/* 10Jan2023, Maiko, Oops, forgot these two - terrible of me !!! */
	sp->send = winrpr_send_stub;
