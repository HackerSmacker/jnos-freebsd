...
/*
 * 18Apr2023, Maiko (VE4KLM), toggle logging of nolistener traffic (ipv4 and ipv6)
 *  (great way to see if your firewall is working or not, or if one of your remote
 *    partners is trying to setup an axudp or axip tunnel with you, or whatever)
 */

int lognolisteners = 0;

static int donolisteners (int argc,char *argv[],void *p)
{
    return setint(&lognolisteners, "Log No Listener Traffic", argc, argv);
}
...
/* 18Apr2023, Maiko (VE4KLM), Toggle logging of IP 'nolistener' traffic */
    { "nolisteners",  donolisteners,  0,      0, NULLCHAR },
#ifdef	RIPAMPRGW
...
#ifdef	DONT_COMPILE
/*
 * 18Apr2023, Maiko, Just move the modified iph_ functions up
 * here instead, added IPV6 support to the iph code just now.
 *
...
static int iph_ceiling = 0;	/* 07May2023, Maiko, default is NO ceiling */

/*
 * Look up an entry in the ip data base
 * 18Apr2023, Maiko, now able to pass either ipv4 or ipv6
 *  (used to be just int32 addr)
 */

static struct iph *iph_lookup (void *ntwkaddr, int ipv6)
{
...
#ifdef	IPV6
		if (ipv6)
			match = (memcmp (ip->addr6, addr6, 16) == 0);
		else
#endif
		match = (ip->addr == *addr);
 ...
 *  (used to be just int32 addr)
 */
static struct iph *iph_create (void *ntwkaddr, int ipv6)
{
	...
	/* 27Apr2023, Maiko, If iph is NULL, then this will blow up on iph->next */
        for(iph = Iph;iph && (iph->next != NULLIPH);last = iph,iph = iph->next);
...
#ifdef	IPV6
	if (ipv6)
		copyipv6addr ((unsigned char*)ntwkaddr, iph->addr6);
	else
#endif
    iph->addr = *(int32*)ntwkaddr;
...
/* 18Apr2023, Maiko (VE4KLM), IPV6 is now part of the heard list */ 
int doipheard(int argc,char *argv[],void *p)
{
    struct iph *iph;

	int showipv4 = 1, showipv6 = 1;
 
	if (argc > 1)
	{
	/*
	 * 18Apr2023, Maiko, For starters, show both ipv4 and ipv6, give a
	 * couple of options : So -4 shows only IPV4, -6 shows only IPV6.
	 *  (easy enough for people to customize for their own needs)
	 */
		if (strstr (argv[1], "-4"))
			showipv6 = 0;
		...
		if (strstr (argv[1], "-6"))
			showipv4 = 0;
	/*
	 * 07May2023, Maiko, Gus (i0ojj) would like to clear counts of
	 * a specific entry, so out of a whim perhaps, why not write a
	 * command to do so. There was also discussion about having an
	 * ip heard packet count ceiling, so any entries in the heard
	 * list would start from zero again if they broke the ceiling.
	 */
		if (strstr (argv[1], "-r"))	/* reset an entry */
		{
			tprintf ("not implemented yet\n");

			return 0;
		}

		if (strstr (argv[1], "-c"))	/* define packet count ceiling */
		{
			if (argc > 2)
				iph_ceiling = atoi (argv[2]);
			else if (iph_ceiling)
				tprintf ("currently %d Pkts\n", iph_ceiling);
			else
				tprintf ("unlimited Pkts\n");
    ...
    for(iph=Iph;iph;iph=iph->next)
	{
		...
		/* 19May2023, Maiko, change padding from 32 to 36 */
        	if (tprintf ("%-36s", ipv6shortform (human_readable_ipv6 (iph->addr6), 0)) == EOF)
				return EOF;
		}
		else
		{
		...
		/* 19May2023, Maiko, change padding from 32 to 36 */
        if (tprintf("%-36s", inet_ntoa(iph->addr)) == EOF)
            return EOF;
#ifdef	IPV6
		}
#endif
		if (tprintf (" %-8s %12s %5ld\n", iph->iface->name, tformat(secclock() - iph->time),iph->count) == EOF)
			return EOF;
    }
    return 0;
}
...
/*
 * 17Apr2023, Maiko (VE4KLM), So I am thinking it seems really wasteful to cut
 * and past functions from IPV4 to IPV6 when really the only thing changing is
 * the object that you are tracking, the code functionality is the same ! Then
 * we can stick to just one function for both network (family) classes, yes.
 * 
 * So similar to how I did nos_log_peerless(), perhaps we should pass a generic
 * pointer to log_ipheard () instead, and use the family (AF_INET vs AF_INET6),
 * to determine what we're pointing at ? 32bit addr or 16 byte unsigned chars.
 *
 * There is no need to add an additional function argument for family, since
 * we can take that from the iface in use, AF_INET6 always comes in via ipv6
 * iface, nowhere else (for the time being anyways) ...
 *
 * 18Apr2023, Maiko, Ha ! This function will look very strange, especially
 * since I can pass the void right down a few levels to lookup and create,
 * originally the argument was int32 addr. BUT, we will have to pass the
 * family (AF_INET or AF_INET6) to lower functions still - that's fine.
 *
 * Make sure the function still works if IPV6 is NOT defined of course.
 */

void log_ipheard (void *ntwkaddr, struct iface *ifp)
{
    struct iph *niph;
#ifdef	IPV6
	int ipv6 = (strstr (ifp->name, "tap") != NULL);
#else
	int ipv6 = 0;
#endif
    if((niph = iph_lookup(ntwkaddr, ipv6)) == NULLIPH)
        if((niph = iph_create(ntwkaddr, ipv6)) == NULLIPH)
            return;
    niph->iface = ifp;
    niph->count++;

	/* 07May2023, Maiko, If ceiling is defined then start at 0 again */
	if (iph_ceiling && (niph->count > iph_ceiling))
		niph->count = 0;
