...
int aprs_send (char *data, int len)
{
	...
	if ((bp = alloc_mbuf (len)) == NULLBUF){
	  if (aprs_debug)
	    aprslog (-1, "aprs_send: Couldn't alloc mbuf (%d)", len);
		return -1;
	}
	...
	if (aprs_debug)
	  aprslog (-1, "sent aprs packet to ourself");
...
int aprs_processing (struct ax25 *hdr, struct iface *iface, struct mbuf *bp)
{
	...
	aprslog( Asocket, "============================================================================");
	aprslog (Asocket, "* Got APRS packet for processing, len %d", len_p(bp));
	...
	if (logon_callsign == (char*)0 || aprs_port == (char*)0){
                aprslog(-1, "aprs_processing, login_callsign or aprs_port not set" );
		return 0;
	}

	len = len_p (bp);

	/* protection against buffer overflow perhaps (should never happen) */
	if (len > 300)
		len = 300;
	...
	if (micepacket && (micemode == 2))
	{
		...
		if ((ptr = process_digis (hdr, ptr)) == (char*)0){
		  if (aprs_debug)
		    aprslog (-1, "aprs_processing: process_digis (1) failed.");
			return 1;
		}
	...
	if ((hdr->dest[0] == aprs_dest[0] && hdr->dest[1] == aprs_dest[1]) ||
		...
		(hdr->dest[0] == id_dest[0] && hdr->dest[1] == id_dest[1]) ||
                micepacket || 1 /* allow altnets */)
	{
		/* If we get in here, then we have a possible APRS frame */

		ptr2 = (char*)aprs_data + 2;	/* skip ax25 ctl and pid fields */

		if( aprs_data[0] != 0x03 || aprs_data[1] != 0xf0 ){
		  /* RF to Internet gate criteria exception #1:
		   *   The packet does not have a control field of 0x03 or a PID of 0xf0.
		   */
		  aprslog( -1, "* invalid control field/PID combo (%02x %02x)", aprs_data[0], aprs_data[1] );
		}
...
#endif
		{
			/* 11Jan2002, Maiko, Attempt at better DTI data validation */
				...
				if (aprs_debug)
					aprslog (-1, "dropping a packet with invalid dti data");
				return 1;
		        }
			...
			if ((ptr = process_digis (hdr, ptr)) == (char*)0){
			        if (aprs_debug)
				  aprslog (-1, "aprs_processing: process_digis (2) failed.");
				return 1;
			}
			...
			/* use UDP to send it to myself (I use UDP to talk inside TNOS) */
			aprslog (-1, "* Calling aprs_send (2)");
			aprs_send (data_APZxxx, len);
		} else {
		  aprslog (-1, "* Got unknown DTI %c destination", *ptr2, pax25( tmp, hdr->dest ));
		  aprslog( -1, "                     source %s => %.*s", pax25( tmp, hdr->source), len, ptr2 );
		}

		return 1;
	} else {
	  if (aprs_debug)
	    aprslog (-1, "aprs_processing: invalid destination %s", pax25( tmp, hdr->dest ));
            aprslog( -1, "                         source %s => %.*s", pax25( tmp, hdr->source), len, aprs_data );
	}

	return 0;
}

#ifdef	APRS_LOGSENT_WZ0C	/* 11Mar2024, Maiko, Make this optional */

void aprs_logsentmbuf( hdr, bp, offset, interface )
struct ax25 *hdr;
struct mbuf *bp;
int offset;
char *interface;
{
  ...
  pullup( &tbp, NULL, offset );
  buflen = pullup( &tbp, buf, len_p( tbp ));

  aprs_logsent( hdr, buf, buflen, interface );
...
void
aprs_logsent( hdr, data, len, interface )
struct ax25 *hdr;
char *data;
int len;
char *interface;
{
  char *headerstr=NULL, *ptr, tmp[10];
  int i;
  FILE *fp;
  char fname[100], date[30];
  time_t nowtime;
  struct tm *tm;
  
  if( hdr ){
    /* need up to:
     * 10 bytes for the source
     *  9 bytes for the dest
     * 10 bytes for each digi
     *  1 byte for the colon
     *  1 byte for the string terminator
     */ 

    ptr = headerstr = (char *)malloc( 10*(2+MAXDIGIS)+1 );

    ptr += sprintf( ptr, "%s>", pax25( tmp, hdr->source ));
    ptr += sprintf( ptr, "%s", pax25( tmp, hdr->dest ));
    for( i = 0; i < hdr->ndigis; i++ ){
      ptr += sprintf( ptr, ",%s", pax25( tmp, hdr->digis[i] ));
      if( hdr->digis[i][ALEN] & 0x80 ){
	ptr += sprintf( ptr, "*" );
      }
    }
    ptr += sprintf( ptr, ":" );
  ...
  sprintf (fname, "%s/xmitlog.txt", APRSdir);
  time (&nowtime);
  tm = localtime( &nowtime );
  strftime( date, 30-1, "%Y%m%d %H%M%S", tm );
  
  if ((fp = fopen (fname, "a+")) != NULLFILE)
    {
      fprintf (fp, "%s [%-10s] ", date, interface );
      if( headerstr ){
...
#endif	/* end of APRS_LOGSENT_WZ0C	*/
