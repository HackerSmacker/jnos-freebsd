...
#endif
#ifdef IPV6
/* 27Mar2023, Maiko (VE4KLM), Would help to log these :] */
    struct j2socketv6 socketv6;
	static char buf[80];
#else
    static char buf[30];
#endif
	...
	/* 27Mar2023, Maiko (VE4KLM), Would help to log these :] */
        case AF_INET6:
            copyipv6addr (sp.in6->sin6_addr.s6_addr, socketv6.address);
            socketv6.port = sp.in6->sin6_port;
            strcpy (buf, pinetv6 (&socketv6));
            break;
#endif
...
#ifdef  EA5HVK_VARA
        case AF_VARA:
            pax25(tmp,sp.va->va_addr);
	    if (sp.va->iface_index != -1)
	      {
		struct iface *ifp = if_lookup2 (sp.va->iface_index);
