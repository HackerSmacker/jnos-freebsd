...
/* 01Aug2023, WZ0C, New 'nwstorf' for gating AE5PL-WX NWS alerts.  Specify three letter NWS weather offices. */
static char **nwstorf = (char**)0;
...
/* 01Aug2023, WZ0C, NWS weather stations to gate to RF */
int callsign_can_nws (char *callsign)
{
	return callsign_in_list (nwstorf, callsign);
}
...
int docallslist (int argc, char *argv[], void *p)
{
	...
	if (!strnicmp (argv[0], j2str_calls, strlen (argv[0])))	/* be more forgiving */
	{
		...
		/* 12Sep2004, Maiko, Added gating of WX info to RF */
			...
			topptr = &wxtorf;
		/* 01Aug2023, WZ0C.  Added gating of NWS alerts to RF */
		else if (!stricmp (argv[1], "nwstorf"))
			topptr = &nwstorf;
