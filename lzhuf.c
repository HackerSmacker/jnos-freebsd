 ...
 *
 * Important information about B2F directive, especially for this file !
 *
 * The code I originally added for B2F is actually used for B1F as well, but
 * did not realize this till well after the B2F code was added, meaning there
 * is a danger in undefining B2F for this particular source file - The safest
 * way to deal with this is to simply make sure B2F is permanently defined in
 * the config.h header file (from this point on).
 *
 * As long as this is documented, I think it will be fine, and there is NO risk
 * in messing up the forwarding code by doing it this way. In the end, the B2F
 * directive will be relegated to simply identify B2F specific pieces of code,
 * and for this particular source file, also B1F specific code :)
 *
 * NEW RULE - #define B2F in config.h is now permanent, not to be changed !
 *
 * IF folks do not want Winlink Secure Login which requires installation of
 * the openssl development linux package, then make sure you have this :
 *
 *    #undef WINLINK_SECURE_LOGIN
 *
 * in your config.h header file, but leave #define B2F alone !!!
 *
 */
...
/*
 * 02Feb2024, Maiko (VE4KLM), Need a debug flag for lzhuf code, so a
 * good opportunity to start my new centralized debugging management
 * routines - getting tired of always having to write new debugging.
 *
 *  So for example, instead of :
 *     if (lzhuf_debug)
 *
 *  We will now use :
 *     if (j2debug (J2DEBUG_LZHUF))
 *
 * Charles was reporting some lzhuf early disconnects, and the code
 * is not clear WHERE in the process things are happening, so fixed
 * this up to maybe give people a better idea (or not) ... sigh ...
 *
 */

// static int lzhuf_debug = 1;
...
/* 01Feb2024, Maiko, Nice to know WHERE it is happening - add extra arg
static char EarlyDisconnect[] = "lzhuf: unexpected disconnect";
 */
static char EarlyDisconnect[] = "lzhuf [%s] unexpected disconnect";

static int sendbuf (int s, char *buf, int len, int mode);
...
#ifdef	DONT_COMPILE
/* declared once again in lzhuf.h (maiko, 14apr2016) */
...
/* 20Feb2016, Maiko (VE4KLM), Added point to unsigned long */
int  Decode    (int, char *, char *, struct lzhufstruct *, int, unsigned long*);
...
/* 20Feb2016, Maiko (VE4KLM), moved out of this function, so
 * that I have more flexible log() options
...
/* 24Mar2008, Maiko (VE4KLM), Added flag for b2f considerations */
/* 20Feb2016, Maiko (VE4KLM), changed 'count' to be passed as argument */
int Decode(int usock, char* iFile, char* oFile, struct lzhufstruct *lzhuf, int b2f, unsigned long *count)
{
   int  i = 0;
   int  j = 0;
   int  k = 0;
   ...
   /* unsigned long int  count      = 0; 20Feb2016, Maiko, decided to pass this as
	an argument to the Decode function, so that I can move the 'log' call out
	of this function, since I want to use logmbox instead */
...
/* 12Oct2019, Maiko, B1F also uses the B2F code in this source file */
   ...
   for(*count = 0; (long)(*count) < filesize; ) {
      pwait(NULL);
      c = DecodeChar(lzhuf);
...
/*
	now that 'count' is passed, I can pull the following code
	from this function and get this information outside this
	function, since lzhuf is passwed also. I'm doing this so
	that I can have more logging flexibility !
	20Feb2016, Maiko (VE4KLM) 
...
/*   short b_checksum;                         // buffer checksum.*/
   ...
   // Encode code.
	...
	/* 20Feb2016, Maiko (VE4KLM), moved out of the Encode () function, so
 	* that I have more flexible log() options out here  */

	else if (f->lzhuf->iFileSize > 0)
	{
		/*
		 * 05May2023, Maiko, interesting how negative result of subtraction
		 * can created such a HUGE number, noticed this several times in the
		 * past, just never did anything about it, till Michael (WZ0C) emailed
	 	 * me, but after some experimenting, all you need to do here, is put
		 * the subtraction into labs() function, also fixed format string.
		 */
		logmbox (usock, f->m->name, "lzhuf compress %ld/%lu = %lu percent",
			f->lzhuf->codesize, f->lzhuf->iFileSize,
				labs (f->lzhuf->iFileSize - f->lzhuf->codesize) * 100L / f->lzhuf->iFileSize);
	}
	...
	*ptr++ = 0;
	...
	sendbuf (usock, buffer, buffer_len + 2, 0);	/* 12Apr2016, Maiko */
...
/*         buffer[x + 2] = ((-b_checksum) & 0xff);   // Store b_checksum.*/
	...
	sendbuf (usock, buffer, x + 2, 0);	/* 12Apr2016, Maiko */
...
/*         f_checksum += b_checksum;*/
   ...
   // Send the EOT
      ...
      sendbuf (usock, buffer, 2, 0);	/* 12Apr2016, Maiko */
...
#ifdef HFDD
/* 02Feb2024, Maiko, The HFDD stuff was supposed to go places, but ??? */
#warning "noting HFDD still defined"
#endif
...
int recv_yapp(int usock, struct fwd *f, char **pzSubject, int32 Timeoutms, int b2f)
{
   ...
   int  oldmode;                             // Socket Mode holder.
	...
	unsigned long count = 0;	/* 20Feb2016, Maiko, used to be INSIDE 'Decode()' */
   ...
   // Set the socket to Binary mode since we'll be receiving Binary data.
	...
	if (j2debug (J2DEBUG_LZHUF))
		log (usock, "recv_yapp timout %d", Timeoutms);
	...
	{
		/* 01Feb2024, Maiko, Extra arg for better debugging */
             log(usock, EarlyDisconnect, "PT");
             NoteError = TRUE;
             break;
         }
         j2alarm(0);
		...
		if (j2debug (J2DEBUG_LZHUF))
			log (usock, "packet type [%d]", packet_type);
        ...
        if ((packet_size = recvchar(usock)) == -1)
		{
         ...
         if (!packet_size)
		{
			/* 02Feb2024, Maiko, Log zero packet sizes */
             log(usock, "zero packet size - using 256");

			 packet_size=256;   /* 0x00 always means 256 */
		}

		if (j2debug (J2DEBUG_LZHUF))
			log (usock, "packet size [%d]", packet_size);
         ...
         if (packet_type == SOH)
		{
			...
			/* 02Feb2024, Maiko, This should be checked for errors */
			if (iFile == NULL)
			{
          	   log(usock, "can't open output file [%s]", f->iFile);
             	NoteError = TRUE;
             	break;
			}
			...
			if (j2debug (J2DEBUG_LZHUF))
				log (usock, "SOH - read %d of %d", recvcnt, packet_size);

            if(recvcnt == -1)
			{
				/* 01Feb2024, Maiko, Extra arg for better debugging */
               log(usock, EarlyDisconnect, "SP");
			...
			else *pzSubject = j2strdup(packet_data);
...
//			log (-1, "do it again - read %d of %d bytes", recvcnt, what2read);
         ...
         else if (packet_type == EOT)
		{
            ...
            if (((-checksum) & 0xff) != (packet_size & 0xff))
			 {
				log (usock, "fbb protocol error, EOT checksum");
                 FBBerror(1,usock);
                 NoteError = TRUE;
	...
	if (j2debug (J2DEBUG_LZHUF))
		log (usock, "close yap file [%s] decode ? %d", f->iFile, !NoteError);
...
/*	pscurproc ();   13Oct2009, Maiko, Log (debug) stack utilization */
	...
	/* 24Mar2008, Maiko (VE4KLM), Added 'b2f' flag for lzw decoding */
         rc = Decode(usock, f->iFile, f->oFile, f->lzhuf, b2f, &count);

         if (rc)
		{
			log (usock, "lzhuf: Decode() error %d",rc);
			++NoteError;
		}
	/*
	 * 20Feb2016, Maiko (VE4KLM), moved out of Decode, able to do that
	 * by passing count as an argument now, and this now gives me a bit
	 * more flexibility in logging - ie, using logmbox instead of log.
	 */
		else if (count > 0)
		{
			/*
			 ...
			 * the subtraction into labs() function, also fixed format string.
			 */
			logmbox (usock, f->m->name, "lzhuf uncompress %lu/%lu = %lu percent",
				f->lzhuf->iFileSize, count,
					labs (count - f->lzhuf->iFileSize) * 100L / count);
		}

      }

	if (j2debug (J2DEBUG_LZHUF))
		log (usock, "decode finished");
...
/*
 * 10Apr2016, Maiko (VE4KLM), send a buffer to a socket, I have to
 * check for IAC and double them up before sending out j2send() or
 * my hfddfixsend() functions. Easy to just write stub function.
 */
static int sendbuf (int s, char *buf, int len, int mode)
{
	int process_iac, newlen = len;
	...
	process_iac = (!(up->flag & SOCK_NO_IAC) && (up->type == TYPE_TCP));

	savptr = ptr = malloc (len + 10);

	while (len > 0)
	{
		*ptr++ = *buf;

		if (process_iac && ((*buf & 0xff) == 0xff))
		{
#ifdef	IACDEBUG
			log (s, "inserting IAC in yapp send");
#endif
			*ptr++ = IAC;
			newlen++;
		}
...
#ifdef	IACDEBUG
	log (s, "sendbuf %d bytes", newlen);
#endif

/* 
 * 12Apr2016, Maiko (VE4KLM), new sendbuf to replace
 * all the j2send and/or hfddfixsend for yapp send.
 */
#ifndef	 HFDD
	j2send (s, savptr, newlen, mode);
#else
	hfddfixsend (s, savptr, newlen);
#endif
	free (savptr);
 ...
 * returning # chars read or -1 if EOF/disconnect
 */
static int recvbuf (int s, char *buf, int len, int32 timeoutms)
{
	int process_iac, c, cnt = 0;
	...
	/*
	 * 10Apr2016, Maiko (VE4KLM), all along I thought for telnet forwarding
	 * one simply had to avoid looking for the IAC in the YAPP payload, but
	 * Bob (VE3TOK) posted something recently which says for FBB forwarding,
	 * you STILL have to look for that IAC and treat it seriously, so let's
	 * give it a shot and see if we can solve once and for all telnet fwd !
	 *
	 * Why am I NOT doing the IAC manipulation deep inside recvchar() ???
	 * 14Apr2016, Maiko (VE4KLM), some thoughts :)
	 *
	 * If I were to do so, then I have to change lots of code, since IAC is
 	 * present in the higher level functions. If I put this inside recvchar
	 * like I really would like to, then I would have to take out the higher
	 * level stuff in the rest of the JNOS source, and right 'now' it's not
	 * worth the trouble (ie, a bit risky as well), so it stays here 4 now.
	 *
	 */
       	if (process_iac && (c == IAC))
		{
        	/* IAC received, get command sequence */
        	c = recvchar(s);

#ifdef	IACDEBUG
			log (s, "detected IAC in yapp recv");
#endif
        	switch (c)
			{
            case WILL:
