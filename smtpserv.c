 ...
 * ----------------------------------------------------------
 *
 * 27Sep2023, Maiko (VE4KLM), Revamped IPV6 start smtp arg processing
 */
...
#ifdef	IPV6
#include "ipv6.h"	/* 23Jun2023, Maiko */
#endif
...
void *p;
{
	...
	/*
	 * 18May2023, Maiko (VE4KLM), support IPV6 !
	 * run both IPV4 and IPV6 smtp if you want.
	 */
	...
	if (argc > 1)
	{
		...
		/* 02Feb2024, Maiko, should have added this 'else' line */
		else port = atoi (argv[1]);

		/* this catches the port if -4 or -6 is specified */
		if (argc > 2)
			port = atoi (argv[2]);
	}
#else
...
#define SMTPSTKALL	(SMTPSTK+SMTPSTK2+SMTPSTK3+SMTPSTK4)
	...
	/* 11Jun2023, Maiko (VE4KLM), don't bother if ipv6iface disabled */
	if (!ipv6iface && doipv6)
	{
		tprintf ("not running smtp ipv6, since ipv6 iface is not defined\n");
		doipv6 = 0;
	}

	/* 18May2023, Maiko, Option now to run both an ipv4 and ipv6 listener */
	if (doipv6)
    	ret = new_start_tcpv6 (port, "SMTP6 Server", smtpserv, SMTPSTKALL, 0);

	if (doipv4)
    	ret = new_start_tcp (port, "SMTP Server", smtpserv, SMTPSTKALL, 0);
 ...
 *
 * 16Sep2023, Maiko (VE4KLM), adding IPV6 support, we could just use a single
 * pointer for IPV4 and IPV6 values, which could save me writing specific code,
 * but leave that for another day ... funtionality first, fancy pointers some
 * other time, same with ipexception_check functions, just create a V6 one for
 * now, and I suppose I could instead make a sdrex6 structure, keeping separate
 * link lists as well, but not sure yet - just make this functional for now !
 */
 ...
 *
 * 21Sep2023, Maiko (VE4KLM), Terrible, should using htoi() - not htol() !
 */
...
#ifdef	IPV6
	copyipv6addr (resolve6 (ip_s), ptr->addr6);

	if (ptr->addr6[0] != 0x00)	/* go to ipv4 if this is 0x00 */
	{
		ptr->netmask6 = htoi (nm_s);
		log (-1, "netmask6 [%x]", ptr->netmask6);
	}
	else
	{
#endif
		ptr->addr = resolve (ip_s);
		ptr->netmask = htoi (nm_s);
		log (-1, "netmask [%x]", ptr->netmask);
#ifdef	IPV6
	}
#endif
}
...
void ipexception_list (struct sdrex **lptr)
{
	...
	for (ptr = *lptr; ptr; ptr = ptr->next)
	{
		if (ptr->addr)
			...
			tprintf ("%s / %d\n", ipv6shortform (human_readable_ipv6 (ptr->addr6), 0), 8 * ptr->netmask6);
	}
}
...
#ifdef	IPV6
/*
 * 16Sep2023, Maiko, IPV6 version of ipexception_check() further below.
 *  (netmask passed as multiple of 8, means 1 byte memcmp for each multiple)
 */
int ipexception_check6 (struct sdrex **lptr, unsigned char *ipaddr6)
{
	struct sdrex *ptr;

	for (ptr = *lptr; ptr; ptr = ptr->next)
	{
		// log (-1, "checking [%s]", ipv6shortform (human_readable_ipv6 (ipaddr6), 0));
		// log (-1, "with [%s] mask [%d]", ipv6shortform (human_readable_ipv6 (ptr->addr6), 0), ptr->netmask6);

	/*
	 * 24Sep2023, Maiko, Remember this is a shared ipv4 / ipv6 list, so we have
	 * to check netmask6 - if it is ZERO, it means this is an ipv4 relay entry.
	 */
		if (ptr->netmask6 && !memcmp (ptr->addr6, ipaddr6, ptr->netmask6))
			break;
	}
...
#ifdef	IPV6
/*
 * 16Sep2023, Maiko, IPV6 version of sdre_check() further below.
 */
int sdre_check6 (unsigned char *ipaddr6)
{
	int retval = ipexception_check6 (&sdre, ipaddr6);

	if (retval)
	{
		log (-1, "smtp [%s] allow relay",
			ipv6shortform (human_readable_ipv6 (ipaddr6), 0));
	}
...
/* rewrite to: address based on <from>|<to> mapping in Refilefile */
        ...
        /* if a local address check for an alias */
            if (address_type == LOCAL) {
                ...
                struct iface *ifp;
			/*
			 * 19May2023, Maiko, This should be fine even for IPV6, since
			 * the length (cnt) is forced to stick with the IPV4 struct.
			 *  (not doing DENY_RELAY or SDR_EXCEPTION for IPV6 as of yet)
			 */
                struct sockaddr_in fsock;
#ifdef	IPV6
                struct j2sockaddr_in6 fsock6;	/* 21Jun2023, Maiko */
#endif
                cnt = sizeof(fsock);
			/*
			 * 19Jun2023, Maiko (VE4KLM), I would like to write a smaller
			 * version of j2getpeername() where I would do the itop() call
			 * external (so I can get the INET family and avoid multiple
			 * calls to that same function for IPV6 purposes, for now I
			 * will use existing code to get family, and reorg code ...
			 */	
                if (!j2getpeername (mp->s, (char*)&fsock, &cnt) && cnt)
				{
					/* just debugging
					 log (mp->s, "deny relay - family %d", fsock.sin_family);
					 */

					if (fsock.sin_family == AF_INET)
					{
					...
					{
						/*
						 * 21Jun2023, Maiko (VE4KLM), for IPV6 don't cycle thru
						 * all the ifaces, there is only one single iface right
						 * at this moment, just compare 'myipv6addr', hardcode
						 * netmask at /120 for now - that's fair ! Nobody will
						 * ever use their full subnet, never mind even a /120,
						 * this also allows smtp from the JNOS linux host :]
						 */
                		cnt = sizeof(fsock6);

                	  if (!j2getpeername (mp->s, (char*)&fsock6, &cnt) && cnt)
					  {
						if (memcmp (myipv6addr, fsock6.sin6_addr.s6_addr, 15))
						{
#ifdef	SDR_EXCEPTIONS
					/* 12Sep2023, Maiko (VE4KLM), Bob wants this for IPV6 */
							if (!sdre_check6 (fsock6.sin6_addr.s6_addr))
							{
#endif
								log (mp->s, "smtp deny relay");
...
#endif
            ap = list;
		/* 27Apr2023, Maiko, If ap is NULL then ap->next will blow up */
            while (ap && (ap->next != NULLLIST)) {
...
/* test if mail address is valid */
    ...
    char *s;
    {
        ...
        int32 addr;
#ifdef IPV6
	/* 09June2023, Maiko, trying to IPV6 smtp client side */
		unsigned char addr6[16];
		extern void mailroute_ipvX (char*, int32*, unsigned char*);
#endif
    ...
    /* if address has @ in it then check dest address */
        ...
        */
			...
			{
#ifdef IPV6
				/* 09Jun2023, Maiko, New mailroute for both ipv6 and ipv4 */
				mailroute_ipvX (cp, &addr, addr6);

				if (*addr6 == 0x00 && addr == 0)
#else
...
#ifdef	IPV6
				/* 09Jun2023, Maiko, Attempt to incorporate ipv6 support */
				if (memcmp (addr6, myipv6addr, 16) || ismyaddr (addr) == NULLIF)
#else
