...
#define IFF_TAP 2
#endif
/* Emulate the Linux way of doing the tun/tap device modes */
...
int tun_attach (int argc, char *argv[], void *p)
{
	...
	/*
	 * 10Jun2023, Maiko (VE4KLM), the loopback device requires a MAC as
	 * well, so any attempt to connect (IPV6) to local host fails if you
	 * don't have it set, enet.c will complaint (protection mech) with :
	 *
            22:49:42  - iface [loopback] hw address not set!
	 *
	 * Might as well do it here, can't think of a better place, right ?
	 *  (knew about this long ago, definitely need localhost IPV6 to work)
	 */

	Loopback.hwaddr = mallocw(6);

	Loopback.hwaddr[0] = 0x54; Loopback.hwaddr[1] = 0x4e;
	Loopback.hwaddr[2] = 0x45; Loopback.hwaddr[3] = 0x00;
	Loopback.hwaddr[4] = 0x00; Loopback.hwaddr[5] = 0x00;
    ...
    /* 20Sep2024, Evie (K4RY), on FreeBSD, don't worry about setting the mode */
