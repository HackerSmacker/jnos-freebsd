 ...
 *
 * 27Sep2023, Maiko (VE4KLM), Now supporting both IPV4 and IPV6 :]
 *
 */
 ...
 */
	...
	{
		tprintf ("not running pop3 ipv6, since ipv6 iface is not defined\n");
		doipv6 = 0;
	}

	/* 27Sep2023, Maiko, Option now to run both an ipv4 and ipv6 listener */
	if (doipv6)
     ret = new_start_tcpv6 (port, "POP3V6 Server", popserv, 1536, 0);

	if (doipv4)
     ret = new_start_tcp (port, "POP3 Server", popserv, 1536, 0);
