...
int rxbroadcast;    /* The only protocol that accepts 'em */
{
	...
	int16 checksum;	/* 04May2023, Maiko */
	 ...
	 *  (it's not working, actually postnote, unsure at the time)
	 *
 	 * 15May2023, Seems to be working now (might always have)
	 */
    checksum = icmpv6cksum (ipv6, bp, len_p (bp));

    if (udp.checksum != 0 && checksum != 0)
    {
	tprintf ("udp checksum error\n");
