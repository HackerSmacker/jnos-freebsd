...
/*
 * 13Jun2023, Maiko (VE4KLM), first attempt IPV6 version of MX lookup
 */

unsigned char *
#ifdef SMTP_A_BEFORE_WILDMX
resolve6_mx (char *name, unsigned char *A_rec_addr6)
#else
resolve6_mx (char *name)
#endif
{
    register struct rr *rrp, *arrp;
    char *sname, *tmp, *cp;
    ...
    while(1){
        ...
        while(rrp != NULLRR)
		{
            if(rrp->rdlength > 0 && rrp->rdata.mx.pref <= pref &&
       	     (addr = resolve6(rrp->rdata.mx.exch)) != 0)
			{
                pref = rrp->rdata.mx.pref;
                copyipv6addr (addr, ip_address);
            }
            rrp = rrp->next;
        }
        free_rr(arrp);
        ...
        if (A_rec_addr[0] || (cp = strchr(cp,'.')) == NULLCHAR)
#else
