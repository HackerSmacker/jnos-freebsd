 ...
 *
 * 06Oct2023, Maiko, Time to try IPV6, do I need to worry about socket pairs ?
 *  Ministry - Live Hellfest 2017 (Full Show HD)' on youtube, and watching my
 *  youngest skating at DAY/JOUR 2 | 2023 Canadian Long Track Championships.
 */
...
static HTTPVNCSESS *getmyhvs (int s, char *orgipaddr)
{
	...
	unsigned char incoming_ipaddr6[16];
#endif
...
#ifdef	IPV6
	{
		copyipv6addr (resolve6 (orgipaddr), incoming_ipaddr6);

		if (incoming_ipaddr6[0] == 0x00)	/* go to ipv4 if this is 0x00 */
#endif
...
#ifdef IPV6
	if (fsocket.sin_family != AF_INET)
	{
		len_i = sizeof(fsocket6);

		if (!j2getpeername (s, (char*)&fsocket6, &len_i) && len_i)
			copyipv6addr (fsocket6.sin6_addr.s6_addr, incoming_ipaddr6);
	}
#endif
	...
	{
#ifdef	IPV6
		if (incoming_ipaddr6[0] != 0x00)
		{
			if (!memcmp (hvs[cnt].ipaddr6, incoming_ipaddr6, 16))
			{
				if (hvsdebug)
					log (s, "found client6 (%d) in session list", cnt);
...
#ifdef	IPV6
		if (incoming_ipaddr6[0] != 0x00)
			copyipv6addr (incoming_ipaddr6, hvs[cnt].ipaddr6);
		else
#endif
...
int httpvnc1 (int argc, char *argv[], void *p)
{
	...
	/*
	 * 06Oct2023, Maiko (VE4KLM), support IPV6 !
	...
	if (!ipv6iface && doipv6)	/* don't bother if ipv6iface is disabled */
	{
		tprintf ("not running httpvnc ipv6, since ipv6 iface is not defined\n");
		doipv6 = 0;
	}

	/* 27Sep2023, Maiko, Option now to run both an ipv4 and ipv6 listener */
	if (doipv6)
     ret = new_start_tcpv6 (port, "HTTP6 VNC Server", serv10000, 8192, 0);

	if (doipv4)
     ret = new_start_tcp (port, "HTTP VNC Server", serv10000, 8192, 0);
