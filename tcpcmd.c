...
/* 12May2023, Maiko, Couple of new vars for tracking tcp access denied packets */
static char *tcpaccessdropstr = "tcp access denied";
unsigned int tcpaccessdropcnt;
...
static int tstat(int flag)
{
	...
	/*
	 * 12May2023, Maiko (VE4KLM), tcp access denied counter, not sure
	 * if there is a proper MIB for this, so use a global counter for
	 * now - two new vars below, although I noticed already tcpOutRsts
	 * counter seems to match, so maybe this is not necessary, leave
	 * it in for now, can always remove it later. My 'ip nolistener'
	 * is now totally quiet again now that tcp access is confirmed
	 * working, good grief !
	 */
	tprintf( "(XX)%-20s%10u\n", tcpaccessdropstr, tcpaccessdropcnt);
/*
    if((j % 2) == 0)
        j2tputs("\n");
 */
	...
	if (tcb->conn.ipver == IPV6VERSION)
	{
        	tprintf(" %s",pinetv6(&tcb->conn.localv6));
        	tprintf("    %s\n",pinetv6(&tcb->conn.remotev6));
	}
#endif
    }
    return 0;
