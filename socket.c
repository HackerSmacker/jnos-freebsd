...
#include "ipv6.h"	/* 07Apr2023, Maiko */

#include "enet.h"	/* 08Apr2023, Maiko, Need for ethernet header */

#ifdef	IPV6
extern struct tcb *open_tcpv6 (struct j2socketv6*, struct j2socketv6*, int, int16, void(*)(struct tcb*, int), void(*)(struct tcb*, int), void(*)(struct tcb*, int, int), int, int);
/* 14Apr2023, Maiko, Better add this new one for UDP sockets */
extern struct udp_cb *open_udpv6 (struct j2socketv6*, void (*)(struct iface*, struct udp_cb*, int));
#endif
...
#ifdef EA5HVK_VARA
int Vai_sock = -1;  /* Socket number listening for VARA connections */
#endif /* VARA */
...
#ifdef	IPV6
	/*
 	 * 11Apr2023, Maiko, At this moment, adding a new 'family'
	 * member to 'struct usock' seems to be the only way I can
	 * identify whether a UDP socket is IPV4 or IPV6. I could
	 * revisit this down the road, but I'm just no seeing it.
	 */
	up->family = af;
#endif
        ...
        switch(type)
	{
        ...
        switch(type){
            ...
            case SOCK_RAW:
				...
				/* 07Apr2023, Maiko */
				if (af == AF_INET6)
	                up->cb.rip = raw_ipv6(protocol,rip_recv);
				else
#endif
	  ...
	  switch(type){
	  case SOCK_STREAM:
	    up->type = TYPE_VARA_STREAM;
            strcpy(up->eol,VARA_EOL);
	    break;
	  case SOCK_DGRAM:
#ifdef SUPPORT_VARA_BROADCAST
	    up->type = TYPE_VARA_BROADCAST;
#else
	    errno = ESOCKTNOSUPPORT;
#endif
	    break;
...
int namelen;    /* Length of name */
{
    ...
    switch(up->type){
        case TYPE_UDP:
#ifdef	IPV6
		/* 11Apr2023, Maiko, new family member allows me to support IPV6 now */
		if (up->family == AF_INET6)
		{
            local.in6 = (struct j2sockaddr_in6 *)up->name;
            copyipv6addr (local.in6->sin6_addr.s6_addr, lsock6.address);
            lsock6.port = local.in6->sin6_port;

			/* 11Apr2023, Maiko (VE4KLM), need a new open_udpv6() for this */
            if ((up->cb.udp = open_udpv6 (&lsock6,s_urcall)) == NULLUDP) {
                errno = EISCONN;
                return -1;
            }
		}
...
int backlog;    /* 0 for a single connection, 1 for multiple connections */
{
    ...
    switch(up->type){
	  ...
	  local.va = (struct sockaddr_va *)up->name;
	  up->cb.vara = open_vara(NULLIF,(char *)&local.va->va_addr,NULLVAADDR,
				  backlog ? AX_SERVER:AX_PASSIVE,0,
				  s_vrcall,s_vtcall,s_vscall,s);
	  break;
#endif
        default:
...
int peernamelen;    /* Length of peer name */
{
...
#ifdef	IPV6
    struct j2socketv6 lsockv6, fsockv6;
#endif
#ifdef  AX25
    struct iface *iface;
#endif
    ...
    // log (-s, "peernamelen %d type %d", peernamelen, up->type);
    ...
    if(up->name == NULLCHAR){
        switch(up->type){
            ...
            case TYPE_RAW:
#ifdef	IPV6
	/* 06Mar2023, Maiko, Need to know the FAMILY, and check
	 * autobind() for support, finally added that 08Mar2023
	 */
		if (up->peername[0] == 10)
              	autobind(s,AF_INET6);
		else
...
#ifdef	IPV6
	/*
	 * 06Mar2023, Maiko, Need to know the FAMILY, different structs
	 */
		if (up->peername[0] == 10)
		{
        /* Construct the TCP-style ports from the sockaddr structs */
            local.in6 = (struct j2sockaddr_in6 *)up->name;
            remote.in6 = (struct j2sockaddr_in6 *)up->peername;
  
            if (!local.in6->sin6_addr.s6_addr[0])
            /* Choose a local address */
                copyipv6addr (myipv6addr, local.in6->sin6_addr.s6_addr);
  
            copyipv6addr (local.in6->sin6_addr.s6_addr, lsockv6.address);
            lsockv6.port = local.in6->sin6_port;
            copyipv6addr (remote.in6->sin6_addr.s6_addr, fsockv6.address);
            fsockv6.port = remote.in6->sin6_port;
  
        /* Open the TCB in active mode */
            up->cb.tcb = open_tcpv6 (&lsockv6,&fsockv6,TCP_ACTIVE,0,
				s_trcall, s_ttcall, s_tscall, 0, s);

		/*
		 * 08Mar2023, Maiko, s_tscall() needs to have na ipv6 counterapr
		 * 09Mar2023, or use the peername value to get the FAMILY, stick
		 * to one function call, saves code duplication, and so on ...
		 */
		}
		else
		{
...
#ifdef	IPV6
	/*
	 * 06Mar2023, Maiko, End of Need to know the FAMILY, different structs
	 */
		}
#endif
	  ...
	  break;
        case TYPE_VARA_STREAM:
	  local.va = (struct sockaddr_va *)up->name;
	  remote.va = (struct sockaddr_va *)up->peername;

            if ((iface = if_lookup2 (remote.va->iface_index)) == NULLIF)
	      {
                errno = EINVAL;
                return -1;
            }

            if(local.va->va_addr[0] == '\0'){
	      char *src_call;
	      /* The local address was unspecified; set it from the TTYCALL address
	       * (if defined) else from the BBSCALL (if defined) else the interface address.
	       */
		...
		src_call = Ttycall;
		  ...
		  src_call = Bbscall;
#endif
	      memcpy(local.va->va_addr,src_call,AXALEN);
	    }

	  up->cb.vara = open_vara(iface,(char *)&local.va->va_addr,(char *)&remote.va->va_addr,
				  VA_ACTIVE,0,s_vrcall,s_vtcall,s_vscall,s);

	  /* Wait for the connection to complete */
	  while((cb.vara = up->cb.vara) != NULLVARA && cb.vara->state != VARASTCON){
#ifdef useblock
	    if(up->noblock){
	      errno = EWOULDBLOCK;
	      return -1;
	  ...
	  if(cb.vara == NULLVARA){
	    free(up->peername);
	    up->peername = NULLCHAR;
	    /* WZ0C - errno is set in open_vara. */

	    return -1;
	  }

	  /* Get local address from the driver. */
	  /* ?? */
...
int *fromlen;       /* Length of peer address */
{
	...
	/*
	 * 12Apr2023, Maiko (VE4KLM), with recv_udp() I think I need to
	 * create a IPV6 version of it, unlike what I did with recv_tcp,
	 * the reason being the IP version specific socket is directly
	 * passed, whereas it's hidden in the tcb struct connection :|
	 *  (look into a single function down the road - not today)
	 *
	 * This new 'family' member for usock ? very handy now !!
	 *
	 */
#ifdef	IPV6
		// log (-1, "TYPE_UDP (recv_mbuf) family %d", up->family);

		if (up->family == AF_INET6)
		{
			// log (-1, "type_udp 1 in recv_mbuf");

       		while((cb.udp = up->cb.udp) != NULLUDP
       		&& (cnt = recv_udpv6 (cb.udp,&fsocket6,&bp)) == -1)
			{
		// log (-1, "about to call pwait(), errno %d", errno);
                 if((errno = pwait(up)) != 0){
					// log (-1, "type_udp 1 pwait errno %d out of loop", errno);
                     return -1;
                 }
        	}
			// log (-1, "type_udp 1 out of loop");
		}
		else
		{
#endif
...
#endif
            if(cb.udp == NULLUDP){
			// log (-1, "recv_mbuf type udp, connection went away");
            /* Connection went away */
                errno = ENOTCONN;
                return -1;
            }
			// log (-1, "recv_mbuf type udp, before type_udp 2");
#ifdef	IPV6
		/* 12Apr2023, Maiko (VE4KLM), IPV6 support for receiving UDP datagrams */
		if (up->family == AF_INET6)
		{
			// log (-1, "type_udp 2 in recv_mbuf");

            if (from != NULLCHAR && fromlen != (int *)NULL && *fromlen >= sizeof(struct j2sockaddr_in6)){
                remote.in6 = (struct j2sockaddr_in6 *)from;
                remote.in6->sin6_family = AF_INET6;
                copyipv6addr (fsocket6.address, remote.in6->sin6_addr.s6_addr);
                remote.in6->sin6_port = fsocket6.port;
                *fromlen = sizeof(struct j2sockaddr_in6);
            }
			/* breakout, saves an else conditional below, at the end anyways */
			break;
		}
#endif
        ...
        case TYPE_RAW:
			...
			/*
			 * 07Apr2023, Maiko
			 * 08Apr2023, Maiko, Nope, peername is null and
			 * will crash JNOS on RAW sockets, so need another
			 * way to figure out if IPV6 or IPV4, new field in
		 	 * the 'struct rip' ...
			 */
			if (cb.rip->ipver == IPV6VERSION)
			{
				/* 08Apr2023, Maiko, Arrggg !!! We have to pull the ethernet
				 * header off first since we're using TAP interface for IPV6.
				 * Noticed this with recv_mbuf() RAW SOCKET in ping stuff :|
				 */
				pullup (&bp, NULLCHAR, sizeof(struct ether));
            	...
            	if(from != NULLCHAR && fromlen != (int *)NULL && *fromlen >= sizeof(struct j2sockaddr_in6))
				{
                	remote.in6 = (struct j2sockaddr_in6*)from;
                	remote.in6->sin6_family = AF_INET6;
                	copyipv6addr (ipv6.source, remote.in6->sin6_addr.s6_addr);
                	remote.in6->sin6_port = 0;
                	*fromlen = sizeof (struct j2sockaddr_in6);
				}
			}
			else
...
#ifdef  EA5HVK_VARA
        case TYPE_VARA_STREAM:
            while((cb.vara = up->cb.vara) != NULLVARA
            && (bp = recv_vara(cb.vara,(int16)0)) == NULLBUF){
#ifdef useblock
                if(up->noblock){
                    errno = EWOULDBLOCK;
                    return -1;
            ...
            break;
        case TYPE_VARA_BROADCAST:
	  /* Not sure this is right yet -- WZ0C */
            while((cb.vara = up->cb.vara) != NULLVARA
		  && (cnt = recv_vara(cb.vara,(int16)0) == NULLBUF)){
#ifdef useblock
                if(up->noblock){
                    errno = EWOULDBLOCK;
                    return -1;
...
#if 0	    
            if(from != NULLCHAR && fromlen != NULLINT && *fromlen >= (int)sizeof(struct sockaddr_va)){
                remote.va = (struct sockaddr_va *)from;
                remote.va->sva_family = AF_VARA;
                memcpy(remote.va->va_addr.node,vahdr.source,AXALEN);
                *fromlen = sizeof(struct sockaddr_va);
	    }
#endif
            break;
#endif /* vara */
...
int tolen;      /* Length of destination */
{
    ...
    struct socket lsock,fsock;
#ifdef	IPV6
	struct j2socketv6 lsock6, fsock6;	/* 11Apr2023, Maiko */
#endif
    int cnt;

/* 21Feb2023, Maiko, is this getting called */
    ...
    if(up->name == NULLCHAR){
        ...
        switch(up->type){
            ...
            case TYPE_RAW:
		/*
		 * 12Apr2023, Maiko, use the new up->family here, this was
		 * preventing a proper local (source) address from being
		 * initialized, note the lack of AF_INET6 support ...
		 *
                autobind(s,AF_INET);
		 */
                autobind(s, up->family);
                break;
#ifdef  AX25
            case TYPE_AX25UI:
                autobind(s,AF_AX25);
	...
	// log (-1, "send_mbuf len %d type %d", cnt, up->type);
    ...
    switch(up->type){
	    ...
	    // log (-1, "before send_udpv6 src %s", human_readable_ipv6 (local.in6->sin6_addr.s6_addr));

            if(to != NULLCHAR)
               	remote.in6 = (struct j2sockaddr_in6 *)to;
	    ...
	    // log (-1, "before send_udpv6 dst %s", human_readable_ipv6 (remote.in6->sin6_addr.s6_addr));

            copyipv6addr (remote.in6->sin6_addr.s6_addr, fsock6.address);
            fsock6.port = remote.in6->sin6_port;

		/* 11Apr2023, Maiko, Will need to write a new send_udpv6() function */
            send_udpv6 (&lsock6, &fsock6, 0, 0, bp, 0, 0, 0);
		}
		else
		{
#endif
...
#ifdef IPV6
			/*
			 * 09Apr2023, Maiko, send_mbuf() RAW socket should also
			 * be IPV6 capable, version is set in rip structure. My
			 * use of directly specifying local ipv6 address in the
			 * way I am doing it is not quite how it's done in ipv4
			 * but can confirm it works for the icmpcmd ping code.
			 */
			if (up->cb.rip->ipver == IPV6VERSION)
			{
				// local.in6 = (struct j2sockaddr_in6*)up->name;
            	...
            	ipv6_send (/* local.in6->sin6_addr.s6_addr */ myipv6addr, remote.in6->sin6_addr.s6_addr,
...
#ifdef EA5HVK_VARA
        case TYPE_VARA_STREAM:
            if((cb.vara = up->cb.vara) == NULLVARA){
                free_p(bp);
                errno = ENOTCONN;
                return -1;
            }
            send_vara(cb.vara,bp);
  
            while((cb.vara = up->cb.vara) != NULLVARA &&
            len_q(cb.vara->txq) * cb.vara->paclen > cb.vara->window){
                if(up->noblock){
                    errno = EWOULDBLOCK;
                    return -1;
                } else if((errno = pwait(up)) != 0){
        ...
        case TYPE_VARA_BROADCAST:
            local.va = (struct sockaddr_va *)up->name;
            if(to != NULLCHAR)
                remote.va = (struct sockaddr_va *)to;
            else if(up->peername != NULLCHAR)
                remote.va = (struct sockaddr_va *)up->peername;
            else {
                errno = ENOTCONN;
            ...
            va_output (if_lookup2 (remote.va->iface_index),
            remote.va->va_addr, local.va->va_addr,0,bp);
            break;
#endif
    }
    return cnt;
...
int rtx;    /* 0 = receive queue, 1 = transmit queue */
{
    ...
    switch(up->type){
        ...
        switch(rtx){
            case 0:
                len = len_p(up->cb.vara->rxq) + len_p(up->ibuf);
                break;
            case 1: /* Number of packets, not bytes */
                len = len_q(up->cb.vara->txq);
                if(up->obuf != NULLBUF)
                    len++;
                break;
        }
...
int how;    /* (see above) */
{
    ...
    switch(up->type){
        ...
        switch(how){
            case 0:
            case 1: /* Attempt regular disconnect */
	      disc_vara(up->cb.vara);
                break;
            case 2: /* Blow it away */
                reset_vara(up->cb.vara);
                up->cb.vara = NULLVARA;
                break;
        }
            break;
...
int s;      /* Socket index */
{
    ...
    switch(up->type){
        ...
        case TYPE_RAW:
			 ...
			 * 08Apr2023, Maiko, Nope, peername is null and
			 * will crash JNOS on RAW sockets, so need another
			 * way to figure out if IPV6 or IPV4 ...
			 */
			if (up->cb.rip->ipver == IPV6VERSION)
				del_ipv6 (up->cb.rip);
			else
...
int old,new;
{
    ...
    switch(new){
         ...
         */
            ...
            /* Clone the socket */
#ifdef IPV6
		log (-1, "clone socket peername [%d]", up->peername[0]);

	/* 09Mar2023, Maiko, Check if this is IPV6 */
		if (up->peername[0] == 10)
                ns = j2socket(AF_INET6,SOCK_STREAM,0);
		else
#endif
	...
	/* 09Mar2023, Maiko, Check if this is IPV6 */
		if (up->peername[0] == 10)
		{
			log (s, "need to load up addresses for IPV6 - won't work yet");
		}
		else
		{
#endif
...
int cnt;
{
  /*  ?? */
    ...
    up = itop(vap->user);
    /* When a VARA connection first comes in, the VARA listener
       is notified, if active. If the AX.25 listener is a server its
       socket is duplicated in the same manner as in s_tscall().
     */
    if (Vai_sock != -1 && vap->user == -1) {
        oup = up = itop(Vai_sock);
        /* From now on, use the same upcalls as the listener */
        vap->t_upcall = up->cb.vara->t_upcall;
        vap->r_upcall = up->cb.vara->r_upcall;
        vap->s_upcall = up->cb.vara->s_upcall;
        if (up->cb.vara->flags.clone) {
            /* Clone the socket */
            ns = j2socket(AF_VARA,SOCK_STREAM,0);
            nup = itop(ns);
            next = nup->next;
            ASSIGN(*nup,*up);
            nup->next = next;
            ...
            nup->cb.vara = vap;
            /* Allocate new memory for the name areas */
            nup->name = mallocw(sizeof(struct sockaddr_va));
            nup->peername = mallocw(sizeof(struct sockaddr_va));
            /* Store the new socket # in the old one */
            up->rdysock = ns;
            up = nup;
        } else {
            ...
            up->cb.vara = vap;
            /* Allocate space for the peer's name */
            up->peername = mallocw(sizeof(struct sockaddr_va));
            /* Store the old socket # in the old socket */
            up->rdysock = Vai_sock;
        }
        /* Load the addresses. Memory for the name has already
         ...
         */
        sp.p = up->name;
        sp.va->sva_family = AF_VARA;
        memcpy(sp.va->va_addr,vap->local,AXALEN);
	/* 30Aug2010, Maiko, New way to reference interface information */
        memset (sp.va->filler, 0, ILEN);
	sp.va->iface_index = if_indexbyname (vap->iface->name);
        up->namelen = sizeof(struct sockaddr_va);
  
        sp.p = up->peername;
        sp.va->sva_family = AF_VARA;
        memcpy(sp.va->va_addr,vap->remote,AXALEN);
	/* 30Aug2010, Maiko, New way to reference interface information */
        memset (sp.va->filler, 0, ILEN);
	sp.va->iface_index = if_indexbyname (vap->iface->name);
        up->peernamelen = sizeof(struct sockaddr_va);
        /* Wake up the guy accepting it, and let him run */
        j2psignal(oup,1);
        pwait(NULL);
        return;
...
int cnt;
{
   ...
   j2psignal(itop(vap->user),1);
   j2psignal(vap->iface->vara->sndq,1);
    pwait(NULL);
}
/* VARA state change upcall routine */
void
s_vscall(vap,old,new)
...
#ifdef	IPV6
/*
 * 21Feb2023, Maiko (VE4KLM), Will need to change this a bit, since
 * the sockaddr structures for IPV4 and IPV6 are not the same size.
 */
			...
			else if (sp.in->sin_family == AF_INET6)
			{
                  if (namelen != sizeof(struct j2sockaddr_in6))
					return -1;
			}
			else return 1;	/* unsupported family */
#else
            if(sp.in->sin_family != AF_INET
       ...
       case TYPE_VARA_STREAM:
	 break;
            if(sp.va->sva_family != AF_VARA
                || namelen != sizeof(struct sockaddr_va))
                return -1;
            break;
#endif
    }
...
int s,af;
{
#ifdef	IPV6
	/*
	 * 09Mar2023, Maiko, Been stung by this a few times, 28 bytes max !!!
	 *
	 * 25Apr2023, Maiko, I am an idiot, you should be using j2sockaddr_in6
	 * you moron, not j2socketv6, GRRRR - Thanks Bob for the traces, he was
	 * getting crashes but only on his 32 bit system, so this is why ...
	 *
    char buf[sizeof(struct j2socketv6)];
	 */
    char buf[sizeof(struct j2sockaddr_in6)];
#else
    ...
    switch(af){
#ifdef	IPV6
	/* 08Mar2023, Maiko (VE4KLM) */
        case AF_INET6:
            sp.in6->sin6_family = AF_INET6;
            zeroipv6addr (sp.in6->sin6_addr.s6_addr);
            sp.in6->sin6_port = Lport++;
            if(!Lport) Lport=1024;  /* wrap to 1024, not zero */
            j2bind(s,sp.p,sizeof(struct j2sockaddr_in6));
            break;
#endif
        case AF_INET:
            sp.in->sin_family = AF_INET;
        ...
        case AF_VARA:
            sp.va->sva_family = AF_VARA;
            memset(sp.va->va_addr,'\0',AXALEN);
	    memset(sp.va->filler,0,ILEN);
	    sp.va->iface_index = -1;

            j2bind(s,sp.p,sizeof(struct sockaddr_va));
	    break;
#endif
    }
}
...
static void i_upcallv6 (struct tcb *tcb, int oldstate, int newstate);

/*
 * 15Feb2023, Maiko (VE4KLM), I must be crazy, but it's that time to
 * give it a try, will it actually work, TCP for IPV6, so far so good
 */
int new_start_tcpv6(port,name,task,stack,flags)
int16 port;
char *name;
void (*task)(int,void *,void *);
int stack;
...
{
    struct inet *in;
    struct j2socketv6 lsocketv6;
    in = (struct inet *)callocw(1,sizeof(struct inet));
    zeroipv6addr (lsocketv6.address);
    lsocketv6.port = port;
    in->tcb = open_tcpv6 (&lsocketv6,NULLJ2SOCKV6,TCP_SERVER,0, NULLVFP((struct tcb*,int)),NULLVFP((struct tcb*,int)), i_upcallv6,0,-1);
    if(in->tcb == NULLTCB) {
        free(in);
        return -1;
    }
...
/*
 * 16Feb2023, Maiko (VE4KLM), write a separate i_upcallv6() for now
 * to get things moving, then break out common code after the fact.
 *
 * Ideas from this nice writeup (2021-06-25) on the IBM website :
 *   (although, not doing dual stack single code yet, still helpful)
 *
    https://www.ibm.com/docs/en/zos/2.4.0?
     topic=applications-example-simple-tcp-server-program-enabled-ipv6
 *
...
static void i_upcallv6 (struct tcb *tcb, int oldstate, int newstate)
{
    ...
    for(in = Inet_list;in != NULLINET;in = in->next)
        if(in->tcb->conn.localv6.port == tcb->conn.localv6.port)
            break;
 ...
 /*21Feb2023, Maiko, time to use AF_INET6 */
    s = j2socket(AF_INET6,SOCK_STREAM,0);
    up = itop(s);

    sock6.sin6_family = AF_INET6;
    copyipv6addr (tcb->conn.localv6.address, sock6.sin6_addr.s6_addr);
    sock6.sin6_port = tcb->conn.localv6.port;

    j2bind (s, (char*)&sock6, sizeof (sock6));
	// log (-1, "j2bind returned %d errno %d", rc, errno);
...
/* 21Feb2023, Maiko (VE4KLM), use the proper ipv6 structures now */
    copyipv6addr (tcb->conn.remotev6.address, sock6.sin6_addr.s6_addr);
    sock6.sin6_port = tcb->conn.remotev6.port;
    up->peernamelen = sizeof (sock6);
    up->peername = mallocw((unsigned)up->peernamelen);
    memcpy(up->peername,(char*)&sock6,sizeof(sock6));
    up->cb.tcb = tcb;
/*
	log (-1, "i_upcall / 21Feb203 - port [%d] user [%02X] flag [%02X]",
		(int)tcb->conn.localv6.port, tcb->user, up->flag);
*/

    /* 21Feb2023, Maiko, We are now here, and ???? */
