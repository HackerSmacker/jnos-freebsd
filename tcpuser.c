...
#ifdef	IPV6
/*
 * 15Feb2023, Maiko (VE4KLM), Again, just paste the entire original open_tcp() function
 * for now, then look at breaking out common code at a later date, just want to get this
 * doing something useful first - netuser.h already contains necessary structures ...
 */
struct tcb *open_tcpv6 (lsocketv6, fsocketv6, mode, window, r_upcall, t_upcall, s_upcall, tos, user)
struct j2socketv6 *lsocketv6; /* Local socket */
struct j2socketv6 *fsocketv6; /* Remote socket */
...
int user;       /* User linkage area */
{
    ...
    if (lsocketv6 == NULLJ2SOCKV6)
	{
        Net_error = INVALID;
        return NULLTCB;
    }

	copyipv6addr (lsocketv6->address, conn.localv6.address);
    conn.localv6.port = lsocketv6->port;

    if (fsocketv6 != NULLJ2SOCKV6)
	{
		copyipv6addr (fsocketv6->address, conn.remotev6.address);
        conn.remotev6.port = fsocketv6->port;
    }
	else
	{
    ...
    if ((tcb = lookup_tcb(&conn)) == NULLTCB)
	{
		...
		{
#ifdef	DONT_COMPILE
/* 15Feb2023, maiko, remember I have no ipv6 routing in place (yet) */
            /* Is this to ourself ? */
            if((ifp=ismyipv6addr(conn.remotev6.address)) == NULLIF)
			{
...
int user;       /* User linkage area */
{
	...
	conn.ipver = IPVERSION;	/* 05Aug2023, Maiko, Darn it all !!! */
...
struct mbuf *bp;
{
  ...
  // log (-1, "send_tcp, ipver %d", tcb->conn.ipver);
