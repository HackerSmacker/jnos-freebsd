 ...
 *
 * 04May2023, Michael Ford (WZ0C) - some corrections to the minimum
 * number of arguments in the cmds array for fldigi, tun, multipsk,
 * agwpe, winrpr, and vara attach commands.
 */
#include <stdio.h>
#ifdef MSDOS
#include <dos.h>
...
#include "j2debug.h" /* 02Feb2024, Maiko (VE4KML), centralized debuggin */

#ifdef KLM_RFTCPDATA
extern int rftcpstart (int, char**, void*);
extern int rftcpstop (int, char**, void*);
#endif
...
#if defined(INP2011) || defined(IPV6)

/*
 * --- network process stack needs to be much higher for specific services ---
 *
 *  30Sep2011, running into stack problems when I introduced INP3 support
 *
 *  12Oct2023, it seems the 'every 4 days, 17 hours' netrom crashes which have
 *  plagued me since I started working on IPV6, are possibly stack related. I
 *  have been running into these bizzare 'heap meets stack, stack meets heap,
 *  or either one meets program global data base pointer 0x10000000' errors,
 *  and so bumping up NWSTACK so far 'seems' to have resolved the problem.
 */

#warning "confirming NWSTACK 4096 - very good"
...
#endif	/* end of INP2011 or IPV6 */

/* daemons to be run at startup time */
struct daemon Daemons[] = {
    { "killer",       1024,   killer },
#ifdef UNIX
    { "timer",        /* 4096 */ 8192,   timerproc },
#ifdef STATUSWIN
...
#ifdef  EA5HVK_VARA
    { "VARA",        vara_send,      vara_output,     pax25,
      setcall,       CL_VARA,        AXALEN },
#endif
...
#ifdef IPV6
/* 09Feb2023, Maiko (VE4KLM), need some commands for IPV6 */
int doipv6 (int argc, char **, void*);
#endif
...
#endif
/*
 * 02Feb2024, Maiko (VE4KLM), centralize the debug flags, tired
 * of always having to write specific to module debug toggles.
 */
    { "debug",   doj2debug,   0, 0, NULLCHAR },
#ifdef DOSCMD
...
#ifdef IPV6
	/* 09Feb2023, Maiko (VE4KLM), We'll need a few basic commands */
    { "ipv6",         doipv6,           0, 0, NULLCHAR },
#endif
#ifdef  MSDOS
...
#ifdef PACKET
#ifndef IPV6
/* 13Mar2023, Maiko (VE4KLM), not using this, just want ethernet support */
...
#ifdef	MULTIPSK
	/* 05Feb09, Maiko (VE4KLM), New interface to MULTI-PSK tcp/ip control */
	{ "multipsk", mpsk_attach, 0, 4,
	"attach multipsk <name> <hostname> <port>" },
#endif	/* End of MULTIPSK */

#ifdef	AGWPE
	/* 23Feb2012, Maiko (VE4KLM), New interface to AGWPE tcp/ip control */
	{ "agwpe", agwpe_attach, 0, 4,
	"attach agwpe <name> <hostname> <port>" },
#endif	/* End of AGWPE */

#ifdef	WINRPR
	/* 02Nov2020, Maiko (VE4KLM), New interface to WINRPR tcp/ip control */
	{ "winrpr", winrpr_attach, 0, 4,
	"attach winrpr <name> <hostname> <port>" },
#endif	/* End of WINRPR */

#ifdef	EA5HVK_VARA
	/* 05Jan2021, Maiko (VE4KLM), New interface to VARA modem */
	{ "vara", vara_attach, 0, 5,
	"attach vara <name> <mtu> <hostname> <port>" },
#endif	/* End of EA5HVK_VARA */
...
#endif
/* 13Oct2022, Maiko (VE4KLM), new TCP data over VARA support */
#ifdef KLM_RFTCPDATA
    { "rftcpsrv",    rftcpstart,  0, 0, NULLCHAR },
#endif
#ifdef  RIP
...
#endif
#ifdef EA5HVK_VARA
    { "vara",        varastart,        SSTACK, 0, NULLCHAR },
#endif
...
/* 13Oct2022, Maiko (VE4KLM), new TCP data over VARA support */
#ifdef KLM_RFTCPDATA
    { "rftcpsrv",    rftcpstop,  0, 0, NULLCHAR },
#endif
...
#ifdef IPV6
/* 31Jan2023, Maiko (VE4KLM), stub for ipv4 and ipv6 dual stack */
extern int ip_preroute (struct iface*, struct mbuf*, int);
#endif
...
/*
 * 02Feb2024, Maiko (VE4KLM), new centralized debug flags. Got tired
 * of having to write individual toggle functions specific to debugs
 * for specific modules, so starting with the new lzhuf_debug flag,
 * and convert all the other debug flags to use this down the road.
 *  (move this new function and code into a new file later)
 *
 * This works quite nicely, so putting it into the development repository
 */

int j2debugflags[NUM_J2DEBUG_FLAGS] = { 0 } ;

static char *j2debugstrs[NUM_J2DEBUG_FLAGS] = { "lzhuf" };

int doj2debug (int argc, char **argv, void *p)
{
	int cnt, flagged = 0;

	for (cnt = 0; cnt < NUM_J2DEBUG_FLAGS; cnt++)
	{
		if (argc == 1)
		{
			if (j2debugflags[cnt])
			{
				tprintf (" %s", j2debugstrs[cnt]);
				flagged++;
			}
		}
		else if (!strcmp (j2debugstrs[cnt], argv[1]))
		{
			if (j2debugflags[cnt])
				j2debugflags[cnt] = 0;
			else
...
int j2debug (int which)
{
	return (j2debugflags[which]);
}
