...
int dtiheard = 0;	/* 02Apr2002, Enable or disable DTI tracking in aprs heard table */

/* 14Nov2002, Maiko, New flag to enable/disable APRS digi feature */
/* 29Mar2003, Maiko, Default any APRS digipeating to NO !!! */
int aprsdigi = 0;

/* 24Sep2023, WZ0C, allow fillin only option for digi */
int digifillin = 0;
...
int32 aprs_is_timeout = 60;			/* every 10 minutes for now */

unsigned char **aprs_is_ringbuf = NULL;
int aprs_is_ringsize = 0;
int aprs_is_ringstart = 0;
int aprs_is_ringend = 0;
int aprs_is_ringoverrun = 0;
int aprs_is_ringmaxfill = 0;
...
static void aprs_igaterun (int, void *, void *);
...
extern char *trimup (char*); /* 09Nov2023, Maiko, Needed for WZ0C patch */
/*
...
extern int callcheck_withssid (char*);   /* 10Mar2009, Maiko, added prototype */

/*
 * 13Nov2023, Maiko (VE4KLM), I have multiple APRS interfaces, so I need
 * to check if a message is actually destined for my system. This has to
 * be done in multiple places in the code, so instead of a cut and paste,
 * just put the code into it's own function, so here is a new function.
 */
int ismymsg (char *callsign)
{
	struct iface *ifp;
	char tmp[AXBUF];
	...
	if (stricmp (callsign, logon_callsign) == 0)
		return 1;
	/*
	 * 
	 * 29Oct2023, Maiko (VE4KLM), Now that I'm wanting to run multiple
	 * APRS interfaces, loop through them to see if the message matches
	 * any of the iface hwaddr (link address) of any of them. I was not
	 * acking incoming messages because I'm not checking for this. And
	 * each iface has a different SSDI (hwaddr) then logon_callsign.
	 */
	...
	for(ifp=Ifaces;ifp->next;ifp=ifp->next)
	{
		...
		{	
			...
			/* 11Nov2023, Maiko, why is it going out 3rd party ? debug */
			if (aprs_debug)
				aprslog (-1, "iface [%s] current [%s]", tmp, callsign);

			if (stricmp (callsign, tmp) == 0)
				return 1;
		}
	}

	return 0;	/* if we get here, it's not destined for us */
}
...
int valid_dti (char dti)
{
	...
	switch (dti)
	{
		case 0x60:  /* Current MIC-E data */
        	case 0x27:  /* Kenwood MIC-E data */
		...
		case 'T':   /* 10Jan2002, Maiko, Oops forgot about APRS telemetry */
        	...
        	case '!':   /* Position without timestamp, or Ultimeter 2000 station */
        	case '#':   /* Peet Bros U-II Weather Station */
   	        case '$':   /* Raw GPS data or Ultimeter 2000 */
         	case '%':   /* Agrelo DF Jr / MicroFinder */
	        case '*':   /* Peet Bros U-II Weather Station*/
 ...
 */
/* 27Sep2023, Changing this function to get rid of the "intertwined reverse
 * return codes", which are confusing and to simplify the code.  
 * Now 1 means forward; 0 means not. 
 */

#define APRS_NOFORWARD   0
#define APRS_FORWARDOK   1

static int message_handler (char *data, int rf)
{
    char callsign[15], *csp, dti;
    char srccall[15], *ssp, *ptr, origsrccall[15];
	char destcall[15], *dsp;
	int cnt, mymsg, srclen = 0, destlen = 0, i, isforwardok=0;
	char digicalls[7][15];
 ...
 thirdparty:
	 ...
	 */
	if (!isalnum (*data)){
                aprslog( -1, "  malformed packet (1)" );

		/* ditch now because the packet is invalid */
	...
	*ssp = 0;	/* make sure you terminate source callsign string */
	...
	if( !isthirdparty ){
	  strcpy( origsrccall, srccall );
	  
	  if( !rf && aprs_heardrecently( srccall )){
	    /* Igate to RF exception criteria #2:
	    ...
	    bad_msg( rf, "Igate2RF exception 2", savebp );

	    /* no ditch, because we may still deliver it locally */
	    isforwardok = APRS_NOFORWARD;
	  }
	}
	
	/*
	 * Make sure we're not seeing our own call.
	 */
	...
	if (!strcmp (srccall, logon_callsign))
	  {
	    if (aprs_debug)
	      aprslog (-1, "dropping a packet originated by us (2)");

	    /* ditch because we don't need to handle packets from this BBS. */
	    /* If the main origination path ever gets moved over to come through
	     * this function, we may need to mark a packet somewhere to require
	     * an ack.  This is currently done in a different path. */
	    return APRS_NOFORWARD;
	  }

	/*
	 * 
	 * 10Nov2023, Maiko (VE4KLM), Now that I'm wanting to run multiple
	 * APRS interfaces, loop through them as well to make sure we don't
	 * see our own call, recall each iface has a different SSID (hwaddr)
	 * then logon_callsign.
	 * 13Nov2023, Maiko, Oops should have been using srccall, not callsign
	 */
        for(ifp=Ifaces;ifp->next;ifp=ifp->next)
		{
            if(ifp->type == CL_AX25)
			{	
				pax25 (tmp, ifp->hwaddr);
				if (strcmp (srccall, tmp) == 0)
				{	
	    			if (aprs_debug)
	      				aprslog (-1, "dropping a packet originated by us (2a)");
...
#else
		if (ismymsg (srccall))	/* 13Nov2023, Maiko, replace with func */
		{
	   		if (aprs_debug)
	...
	if (rf && callsign_ignored (srccall))
	{
		...
		/* ditch because we're not going to deliver this packet at all. */
		return APRS_NOFORWARD;
	}
	 ...
	 * 31Jul2023, WZ0C, Moving this up here with the other srccall acceptance
	 * check.
	 */
	if (strnicmp (srccall, "aprs", 4) == 0){
		if (aprs_debug)
			aprslog (-1, "destination aprs ignored [%s]", srccall);

		/* ditch now because this isn't a packet to be delivered */
		return APRS_NOFORWARD;
	}
	...
	while (*data && *data != ':' && *data != ',' ){
	  if( destlen > 9 )
	    {
	      bad_msg( rf, "Destination Callsign", savebp);
	      aprslog( -1, "  malformed packet (3)" );
	...
	*dsp = 0;
	...
	while( *data )
	{
	  ...
	  /* Digi list consists of:
	   * alphanumerics in the callsigns
	   * hyphen to separate SSID
	   * slashes for repeaters, mobiles, etc.
	   * asterisk to signify being repeated
	   * comma to separate digis
	   * atsign ??
	   * colon to end the digi list
	   */
		    ...
		    (*data != '*') && (*data != ',') && (*data != '@') && (*data != ':'))
		{
			...
			/* ditch because we got an invalid character */
			return APRS_NOFORWARD;
		}

		if( isalnum( *data ) || (*data == '-') || (*data == '/') || *data == '*' ){
		  if( digilen > 10 ){  /* 10 to account for the possible '*' at the end */
		    bad_msg( rf, "Digi Callsign", savebp );
		    aprslog( -1, "  malformed packet (4)" );

		    /* ditch because the callsign was too long */
		    return APRS_NOFORWARD;
		  }
		  *dsp++ = *data++;
		  digilen++;
	...
	/* Look for TCPIP or TCPXX in the header.  These are needed to determine whether
	 * to pass between RF and APRS-IS.  */

	if( !strncmp( destcall, "TCPIP", 5 ))
	  hastcpip = 1;
	else if( !strncmp( destcall, "TCPXX", 5 ))
	  hastcpxx = 1;
	else if( !strncmp( destcall, "NOGATE", 6 ))
	  hasnogate = 1;
	else if( !strncmp( destcall, "RFONLY", 6 )){
	  hasrfonly = 1;
	} 

	for( i = 0; i < numdigis; i++ ){
	  if( !strncmp( digicalls[i], "TCPIP", 5 ))
	    hastcpip = 1;
	  else if( !strncmp( digicalls[i], "TCPXX", 5 ))
	    hastcpxx = 1;
	  else if( !strncmp( digicalls[i], "NOGATE", 6 ))
	    hasnogate = 1;
	  else if( !strncmp( digicalls[i], "RFONLY", 6 )){
	    hasrfonly = 1;
	  }
	}


	/* Check some of the APRS-IS forwarding rules. */
	
	if( rf ){
	  if( isthirdparty ){
	    if( hastcpip || hastcpxx ){
	      /* RF to Internet igate exception #3:
	       * 3rd-party packets (data type } ) with TCPIP or TCPXX in the 3rd party header.
	       */
	      
	      bad_msg( rf, "RF2igate exception 3", savebp );

	      /* don't ditch yet because we may need to deliver this locally */
	      isforwardok = APRS_NOFORWARD;
	    }  else {
	      char tmpbuf[300];
	      /* RF to Internet igate rule #3:
	       * 3rd-party packets without TCPXX or TCPIP mnust have the RF header and the 3rd party
	       * data type stripped before passing to APRS-IS.
	       */
	      
	      /* strip everything up to and including the '}'. 
	       * Keep in mind that the source and destination buffers overlap.
	       */

	      aprslog( -1, "* RF2igate rule 3: stripping header" );
	      strcpy( tmpbuf, tpstart );
	      strcpy( bufstart, tmpbuf);
	    }
	  } else {
	    if( hastcpip || hastcpxx || hasnogate || hasrfonly ){
	      /* RF to Internet igate exception #5:
	       * packets with TCPIP, TCPXX, NOGATE, or RFONLY in the header (last 2 are optional).
	       */

	      bad_msg( rf, "RF2igate exception 5", savebp );
	...
	if( !rf && !isthirdparty ){
	  if( hastcpxx || hasnogate || hasrfonly ){
	    /* Igate to RF exception criteria #3:
	     * The sending station does not have TCPXX, NOGATE, or RFONLY in the header.
	     */
	    
	    bad_msg( rf, "Igate2RF exception 3", savebp );
	...
	if (!(*data))
	{
		...
		/* ditch now because the packet has no data */
		return APRS_NOFORWARD;
	}
	...
	if( rf && dti == '?' ){
	  ...
	  bad_msg( rf, "RF2igate exception 4", savebp );

	  /* don't ditch because we may still have to deliver this */
	  isforwardok = APRS_NOFORWARD;
	}
	 ...
	 */
	if (!valid_dti (dti) && dti != '}' )
	{
		bad_msg (rf, "Bad or missing DTI", savebp);

		/* ditch because the packet is invalid */
		return APRS_NOFORWARD;
	}

	/* Go back through for third party traffic */
	if( dti == '}' && !isthirdparty ){
	  aprslog( -1, "* Got third party message. Looping back through." );
	  isthirdparty = 1;
	  tpstart = data;  /* this points to the first character after the }  */
	...
	if (!valid_dti_data (dti, data))
        {
                bad_msg( rf, "Bad DTI data", savebp );
...
#if 0
	/* Not sure this is needed now, since we're assuming forwarding is okay,
	 * unless a rule turns it off. */
	if( rf )
	  isforwardok = APRS_FORWARDOK;
#endif

	/* The APRS-IS gating rules mostly cover messages.  So we'll set the default
	 * to not forward for other types. */

	if( !rf && dti != ':' && !dti_pos_or_stat( dti ))
	  isforwardok = APRS_NOFORWARD;
	
	/* Don't forward packets to RF for other Igates.  They can do it themseles. */
	if( !rf && !isthirdparty && aprs_getstatus( srccall ))
	  isforwardok = APRS_NOFORWARD;
	...
	if (dti == '!' || dti == '=' )
	  aprs_db_add_position (srccall, data + 0);

	if( dti == '/' || dti == '@' )
	  aprs_db_add_position( srccall, data + 7 );
#endif
	...
	if (dti == ':') /* Direct or 3rd Party Message */
    {
        ...
        while (*data && *data != ' ' && *data != ':')
		{
			...
			if (srclen > 9)	/* safety feature, incase we get garbage */
			{
		      ...
		      /* ditch now because the packet is not valid */
		      return APRS_NOFORWARD;
#if 0
				if (rf)
			return (1);  /* Don't forward to RF. */
				else
			return (0);  /* Don't forward to Internet. */
#endif
			}
        ...
        *csp = 0;   /* terminate callsign string */
		...
		/* 31Jul2023, WZ0C, Moving this to be included with the associated parsing. */
		while (*data && *data != ':')
		  data++;
		...
		/* callsign has been extracted now */

		aprslog (Asocket, "* callsign: %s", callsign);

		if( !rf ){
		  if( !aprs_heardrecently( callsign ) && strncmp( callsign, "BLN", 3)){
		    /* Igate to RF exception criteria #1:
		     ...
		     */
		    /* Igate to RF exception criteria #4:
		     * The receiving station has not been heard via the Internet within a 
		     * predefined time period.
		     */

		    /* These two above can be combined for now.  If the station is heard
		     * on the Internet, it's removed from the RF heard list. */

		    bad_msg( rf, "Igate2RF exception 1", savebp );

		    /* don't ditch because we may need to deliver this packet locally */
		    isforwardok = APRS_NOFORWARD;
		  }
		}
		...
		if( hastcpip || hastcpxx ){
		  /* APRS-IS:
		   * A station is said to be heard via the Internet if:
		   */
		...
		/* Allow nearby bulletins to gate to RF */
		if( !rf && !strncmp( callsign, "BLN", 3 ) && !aprs_withindistance( srccall, 100.0 ))
		  isforwardok = APRS_NOFORWARD;


		/* If srccall is allowed to send a message to another station, it's also allowed
		 * to send a position report. */
		if( !rf && strncmp( callsign, "BLN", 3 ) && isforwardok == APRS_FORWARDOK )
		  aprs_addstatexception( srccall );
		
		
		/*
		 * Start doing checks for various types of messages.
		 */

		/* Process a bulletin, if this is one. */
		aprs_bulletincheck( srccall, callsign, data );
		
		/* Process an ack, if this has one. */
		aprs_ackcheck( srccall, callsign, data );
        ...
        for(ifp=Ifaces;ifp->next;ifp=ifp->next)
		{
			...
			{	
				...
				if (strcmp (callsign, tmp) == 0)
					mymsg = 1;
			}
		}
#else
		mymsg = ismymsg (callsign);	/* 13Nov2023, Maiko, replace with func */

#endif	/* end of DONT_COMPILE */

		/* 11Nov2023, Maiko, move this to the aprs log */
		if (mymsg && aprs_debug)
			aprslog (-1, "this is my message");
		...
		for (cnt = 0; cnt < MAXANETCLI; cnt++)
		{
			...
			if (aptr->server && aptr->flags == IGATE_ONLINE)
			{
				...
				if (cptr->user && stricmp (callsign, cptr->user) == 0)
				{
					if (aprs_debug)
					...
					sprintf( header, "%s>%s::", srccall, callsign );
					
					if (aprs_debug)
					  aprslog (-1, "header [%s] data [%d] [%s]", header, strlen (data), data);

					if (j2send (cptr->s, header, strlen (header), 0) < 0)
						aprslog (-1, "unable to send header to client");
					...
					if (rf)
						...
						return (0);
#endif
					/* Even if we deliver it locally, send it to the igate for tracking. */
					if( rf )
					  isforwardok = APRS_FORWARDOK;
					else
					  isforwardok = APRS_NOFORWARD;
		...
		if (!rf && !mymsg)
		{
		  if( digicalls > 0 && !strcmp( digicalls[numdigis-1], "AE5PL-WX" )){
		    if( !callsign_can_nws( srccall ) || callsign[3] == '_' )
		      rfallowed = 0;
...
#endif
		if( aprs_debug ){
		  aprslog( -1, "  RF %sallowed", rfallowed?"":"not " );
		}
		

		aprslog (Asocket, "* Data (2): %s", data);
		...
		if (emailmode && rf && strcmp (callsign, "EMAIL") == 0)
		{
			if (emailmode == 1)
			{
				 ...
				 * duplicate email requests.
			 	 */
				dupe = aprs_dup (srccall, destcall, data, strlen( data ), 300 );
				...
				/* leaving because we've handled this one locally */
				return APRS_NOFORWARD;
#if 0				
			...
			if (emailmode == 2)
			{
				...
				/* leaving because we're not going to handle this one */
				return APRS_NOFORWARD;
#if 0
				return (1);
#endif
		...
		if (!mymsg)
		{
			...
			/* name is lower case, callsign is uppercase, use strcasecmp */
			  ...
			  aprslog (Asocket, "* Checking mbox %s vs %s", mbxptr->name, callsign);


			  /* use this to match against the the chosen SSID of the user. */
				if (aprs_checkuserssid (mbxptr->name, callsign))
					break;
        	}
#else
			for (cnt = 0; cnt < NUMMBX; cnt++)
			{
				...
				{
				/* name is lower case, callsign is uppercase, use strcasecmp */
					if (aprs_checkuserssid (Mbox[cnt]->name, callsign))
						break;
				}
			}
			...
			if (mbxptr != NULLMBX)
			{
				...
				/* We may need to deliver it still to another session. */
				isforwardok = APRS_NOFORWARD;
#if 0
		...
		{
		  //			if (!rf)
		  //			  isforwardok = APRS_FORWARDOK;  /* Okay to send to RF */
		}
		else	
		{
		  //			if (rf)
			 isforwardok = APRS_NOFORWARD;  /* Not okay to send to Internet */
	...
	else if (dti_pos_or_stat (dti))
	{
		...
		if (!rf && isforwardok == APRS_FORWARDOK )
		{
		  isforwardok = APRS_NOFORWARD;
			...
			else if( aprs_withindistance( srccall, 15.0 ))
			  isforwardok = APRS_FORWARDOK;

			else if( aprs_hasstatexception( srccall ))
			  isforwardok = APRS_FORWARDOK;
		}
	}
	...
	{
	  if( !rf && digicalls > 0 && !strcmp( digicalls[numdigis-1], "AE5PL-WX" )){
	    aprslog( -1, "  Found weather item" );
	    isforwardok = APRS_FORWARDOK;
...
#endif
	if( aprs_debug ){
	  aprslog( -1, "  Forward to %s %sallowed", !rf?"RF":"Internet", isforwardok?"":"not " );
	}
...
static void aprs_net (int unused OPTIONAL, void *sp, void *p OPTIONAL)
{
	...
	while (1)
	{
		...
		if ( message_handler ((char*)UDPbuf, MSG_VIA_RF) == APRS_FORWARDOK )
		{
			KLMWAIT (NULL);	/* give other processes a chance */
...
/* 11Mar2024, Maiko, Make this optional */
			...
			aprs_logsent( NULL, (char *)UDPbuf, foo, "igate" );
#endif
			if( !aprs_txenable ){
			  aprslog( -1, "  Not sending to igate.  aprs tx disabled." );
			  continue;
			}
		  ...
		  if( aprs_debug )
		    aprslog( Asocket, "  message_handler: don't send to igate" );
		}
#endif
	}
...
static void aprsx (int unused OPTIONAL, void *u OPTIONAL, void *p OPTIONAL)
{
	...
	/* load data for bulletins */
	ret = loadstates();
	if( ret == -1 ){
	  aprslog( -1, "couldn't load state list" );
	}

	ret = loadsamecodes();
	if( ret == -1 ){
	  aprslog( -1, "couldn't load SAME codes" );
	}
    ...
    while (1)
    {
...
#else
#ifdef  TNOS241
        (void) kpause (__FILE__, __LINE__, 10000L); /* 10 seconds */
#else
        (void) kpause (10000L);
#endif
#endif
        ...
        aprs_is_ringsize = 1024;
	aprs_is_ringbuf = (unsigned char **)malloc( sizeof(unsigned char *) * aprs_is_ringsize );
	ptr = (unsigned char *)malloc( sizeof(unsigned char) * aprs_is_ringsize * 300 );

	for( i = 0; i < aprs_is_ringsize; i++ )
	  aprs_is_ringbuf[i] = ptr + 300 * i;

	aprs_is_ringstart = 0;
	aprs_is_ringend = 0;

	/* start the queue runner */
	(void) newproc ("APRS Server", 2048, aprs_igaterun, 0, NULL, NULL, 0);
	...
	while (1)
	{
		...
		while (1)
		{
			...
			buffer = aprs_is_ringbuf[aprs_is_ringend];
			if ((len = recvline (Hsocket, (char*)buffer, 300 - 2)) == -1)
			{
			...
			/* accept the buffer into the ring */
			aprs_is_ringend++;

			if( aprs_is_ringend >= aprs_is_ringsize )
			  aprs_is_ringend = 0;
			if( aprs_is_ringend == aprs_is_ringstart ){
			  aprs_is_ringoverrun++;
			  if( aprs_is_ringoverrun % 10 == 0 )
			    aprslog( -1, "aprs_is_ringoverrun = %d", aprs_is_ringoverrun );
			}
			ringfill = aprs_is_ringend - aprs_is_ringstart;
			if( ringfill < 0 )
			  ringfill += aprs_is_ringsize;
			if( aprs_is_ringmaxfill < ringfill )
			  aprs_is_ringmaxfill = ringfill;

			if( nxtsrv->in_packets % 10000 == 0 )
			  aprslog( -1, "aprs_is_ringmaxfill = %d/%d", aprs_is_ringmaxfill, aprs_is_ringsize );
		}
...
static void aprs_igaterun (int unused OPTIONAL, void *u OPTIONAL, void *p OPTIONAL)
{
  unsigned char *buffer;
  int len;
  
  while(1){
    if( aprs_is_ringstart == aprs_is_ringend ){
      j2pause( 1000 );
      continue;
    }

    buffer = aprs_is_ringbuf[aprs_is_ringstart++];
    if( aprs_is_ringstart == aprs_is_ringsize )
      aprs_is_ringstart = 0;
    ...
    if (message_handler ((char*)buffer, MSG_VIA_IGATE) == APRS_FORWARDOK )
      {
	aprslog( Asocket, "  message_handler: send to RF" );
	KLMWAIT (NULL);	/* give other processes a chance */
	...
	if (convert_3rd_party (buffer) != -1)
      ...
      if( aprs_debug )
	aprslog( Asocket, "  message_handler: don't send to RF" );
    }
	...
	while (1)
	{
		...
		if (len > 1)
		{
			...
			if (*outdata == 0 || *outdata == '\n' || *outdata == '\r'){
			  if( *cp == ':' ){
                                aprs_checkifneedsack( 0, outdata + 1, len - 1 );
				aprs_send (outdata + 1, len - 1);
			  } else {
			    sendrf (outdata + 1, len - 1);
			  }
			} else {
			  if( *cp == ':' ){
                                aprs_checkifneedsack( 0, outdata, len );
...
static int doflags (int argc, char *argv[], void *p OPTIONAL)
{
	...
	if (argc == 1)
	{
		tprintf ("%cdebug %cbadpackets %cmonitor %cpoe %cdtiheard %cbconlyifhrd %cdigi %cfillindigi %cdebugdigi %caprs_txenable\n",
			aprs_debug ? '+' : '-', aprs_badpackets ? '+' : '-',
			monitor_aprsnet ? '+' : '-', aprs_poe ? '+' : '-',
			dtiheard ? '+' : '-', bconlyifhrd ? '+' : '-',
			 aprsdigi ? '+' : '-', digifillin ? '+' : '-',
			 debugdigi ? '+' : '-',  aprs_txenable ? '+' : '-' );
	}
	else
	{
		while (argc > 1)
...
static int dolocmsg (int argc, char *argv[], void *p OPTIONAL)
{
	...
	if (argc == 4)
	{
		...
		/*
		 * 13Nov2023, Maiko, replace with func to check ALL iface calls
		 *   if (strcmp (srccall, logon_callsign))
		 */
		if (!ismymsg (srccall))
	...
	/* does this packet require an acknowledgement? */
	if((c = strchr( argv[2], '{' ))!=NULL){
	  c++;
	  for( len = 0; len < 5 && *c != '}' && *c != '\0'; len++ ){
	    msgno[len] = *c++;
	  }
	  msgno[len] = '\0';

	  aprs_needsack( 1, msgbuf, strlen( msgbuf ), srccall, argv[1], msgno );
	}
...
static int dointmsg (int argc, char *argv[], void *p OPTIONAL)
{
	...
	if((c = strchr( argv[2], '{' ))!=NULL){
	  ...
	  aprs_needsack( 0, msgbuf, strlen( msgbuf ), srccall, argv[1], msgno );
	}
      ...
      struct ip *ip OPTIONAL;
{
	...
	if (*ptr == '}' || *ptr == ':')
	{
		...
		if (dti_pos_or_stat (dti_c) ||
			...
			mice_dti (dti_c) ||
#endif
		/* 21Dec2008, Maiko, Add objects and items to the stuff we igate */
			dti_c == ';' ||
                        dti_c == ')' ||
			dti_c == '_')
		{
	 ...
	 */
	if (!callcheck_withssid (rtmp))
	{
                aprslog (-1, "callcheck (%s) - probable bulletin %s", rtmp, isthirdparty?"(thirdparty)":"");
	...
	if (setcall (dest, (isthirdparty&0)?"APRS":((char*)aprsdestcall)) == -1)
	{
	  aprslog (-1, "setcall failed [%s]", (isthirdparty&0)?"APRS":aprsdestcall);
		return;
	}
...
#endif
	aprslog( -1, "  recipient = %s", pax25( tmp, recipient) );
	...
	if ((strcmp (rtmp, aprsdestcall) && axr == NULLAXR) || (!strcmp(rtmp,aprsdestcall) && isthirdparty))
	{
#ifdef	JNOSAPRS
...
#endif
		aprslog( -1, "  recipient = %s", pax25( tmp, dest) );
	}

	memcpy (addr.dest, dest, AXALEN);

	/*
	 * 01Oct2023, Maiko (VE4KLM), Aprscall should only be used for APRS
	 * Internet System. Should be using the IFACE hwaddr for ANY traffic
	 * going out my RF ports, not the Aprscall, which will confuse systems
	 * because they hear VE4KLM on HF Packet, but also hear VE4KLM on RPR,
	 * so an Ack or Reply winds up being heard on the 'wrong' port, many
	 * times in 3rd party format, since it was a different freq/mode.
	 *
	memcpy (addr.source, Aprscall, AXALEN);
	*/

	memcpy (addr.source, iface->hwaddr, AXALEN);
...
/* 11Mar2024, Maiko, Make this optional */
#ifdef	APRS_LOGSENT_WZ0C
	  aprs_logsent( &addr, bp->data, bp->cnt, iface->name );
#endif
	/* send the packet out !!! */
...
/* 11Mar2024, Maiko, Make this optional */
	  ...
	  aprs_logsent( &addr, bp->data, bp->cnt, iface->name );
#endif
	  aprslog( -1, "  Not sending to RF.  aprs tx disabled" );
	  free_p( data );
	}
...
int dombaprs (int argc OPTIONAL, char *argv[] OPTIONAL, void *p)
{
	...
	/* increasing size of srccall from 20 to 34 to get rid of compiler warnings */
	char tmpbuf[20], srccall[34], recipient[20], cmd_c;
	char *huhstr = "Huh ?\n", *pingstr = "?APRST";
	char *mymsgargv[4], *cp;
	void *vp = (void*)0;
    ...
    struct iface *iface;	/* 09Nov2023, Maiko, find current APRS iface */

	/*
	 * 12Nov2023, The mailbox information is useless if APRSC is run from
	 * the JNOS console, so we can't depend on things like m->name, etc.
	 * Some of the new features put in by WZ0C require m->name, so create
	 * a new variable 'username' for him to use, default to APRS logon call
	 * if run from JNOS console, or set to m->name if run from BBS prompt.
	 */
    char *username;

	if (isconsole)
   		username = j2strdup (logon_callsign);
	else
		username = m->name;
	...
	if (!isconsole)
	{
	  ...
	  /* append SSID if the user has opted for one */
	  ssid = aprs_getuserssid( m->name );
	  if( ssid != -1 )
	    sprintf( srccall, "%s-%d", m->name, ssid );
	  else
	...
	{
		/*
		 * 09Nov2023, Maiko (VE4KLM), in order for this to work
		 * properly with WZ0C's new aprs patch, and the fact I
		 * am now setting the srccall in outgoing APRS frames
		 * to the hwaddr of the 'current' APRS interface, we
		 * should be using the hwaddr of said interface now.
		 *
		strcpy (srccall, logon_callsign);
		 */

		/* find the current APRS port, get the port's ax25 address */
		for (iface = Ifaces; iface != NULLIF; iface = iface->next)
		{
			if (stricmp (iface->name, aprs_port) == 0)
				break;
		}
		if (iface != NULLIF)
			strcpy (srccall, pax25 (tmpbuf, iface->hwaddr));
		else
			strcpy (srccall, logon_callsign);
	}

	/*
	 * 12Nov2023, Maiko, Oops, 'm' does not exist for console - Michael
	 * assumes it does (further down with the new /s command, so actually
	 * this is a good catch, explains why there is BLANK-SSID going on if
	 * you run aprsc from JNOS console.
	 *
	 * I should have seen this WAY BACK when I wrote it :(
	 *
	 * Note to self 'm' is not valid if this is called from console,
	 * so do NOT depend on things like m->name or m->whatever ...
	 *
	aprslog (-1, "srcall (console) name [%s] src [%s]", m->name, srccall);
     *
	 */
	...
	*recipient = 0;
	...
	while (1)
	{
		 ...
		 */
			...
			{
				j2tputs ("/b - show current APRS bulletins.\n");
#ifndef	APRSC
				/* 30May2002, VE4KLM, Option to force msgs out RF !!! */
				j2tputs ("/f - toggle msg routing - RF or APRS IS.\n");
#endif
			        j2tputs ("/k [on|off] - keep receiving messages when not logged in\n" );
			        j2tputs ("/l - set latitude and longitude\n" );
			...
			else if (cmd_c == 'k')
			{
			  ...
			  if( *cp == '\0' ){
			    /* show status */
			    att = aprs_getattribute(username /* m->name */, "keeprx" );
			    if( att && !strcmp( att, "on" ))
			      status = 1;
			    else
			      status = 2;
			  } else if( !strcasecmp( cp, "on" )){
			    aprs_setattribute(username /* m->name */, "keeprx", "on" );
			    status = 1;
			  } else if( !strcasecmp( cp, "off" )){
			    aprs_setattribute(username /* m->name */, "keeprx", "off" );
			    status = 2;
			  } else {
			    j2tputs( "Please specify on or off.\n" );

			    continue;
			  }

			  if( status == 1 )
			    j2tputs( "BBS will continue to receive APRS messages when you're not logged in\n" );
			  else
			    j2tputs( "BBS will not receive APRS messages when you're not logged in\n" );
			...
			else if (cmd_c == 'b' )
			{
			  if( *cp == '\0' )
			    ...
			    /* /b # text 
			     * set my bulletin text 
			     * 
			     * /b clear #
			     * clear my bulletin
			     *
			     * /bg # group text 
			     * set my group bulletin text 
			     * 
			     * /bg clear # group
			     * clear my group bulletin
			     *
			     * /b mine
			     * show my bulletins
			     */
			...
			{
			  /* Allow the user to set an SSID.  If they use this option
			   * with a number (1-15), it will set that as the SSID.  If 
			   * no number is supplied, it will show the current SSID. */
			  ...
			  if( *cp == '\n' || *cp == '\0' ){
			    ssid = aprs_getuserssid(username /* m->name */);
			    if( ssid == -1 ){
			      tprintf( "No SSID set.\n" );
			    } else {
			      tprintf( "Using SSID -%d\n", ssid );
			    }
			    continue;
			  }
			  
			  ssid = aprs_setuserssid(username /* m->name */, cp );
			  if( ssid != -1 ){
			    sprintf( srccall, "%s-%d", username /* m->name */, ssid );
			    tprintf( "Setting call to %s\n", srccall );
			  } else {
			    j2tputs( "SSID must be between 1 and 15\n" );
			  }
		aprslog (-1, "srcall (console) name %s src %s", username, srccall);
		...
		/* 03Oct2001, Concatenate a msg ID to end of message if enabled */
