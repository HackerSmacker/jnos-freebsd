 ...
 *  (27Sep2023, Maiko, Did a revamp of how the args are processed)
 *
 */
#include <ctype.h>
#include "global.h"
...
int telnet1 (int argc, char **argv, void *p)
{
    ...
    int flags = 0;	/* 19Apr2016, Maiko (VE4KLM), new flags */

/*
 * 02Feb2024, Maiko, Debugging to figure out messed up ports
 *
	for (port = 0; port < argc; port++)
		tprintf (" [%s]", argv[port]);
 ...
 */
	...
	int ret, doipv4, doipv6, currarg = 1;	/* 27Sep2023, Maiko, currarg new */

	doipv4 = doipv6 = 1;	/* default both ipv4 and ipv6 listeners */

	port = IPPORT_TELNET;

	/* 08Jul2023, Maiko, run both IPV4 and IPV6 telnet if you want */
    if (argc > 1)
    {
        if (strstr (argv[currarg], "-4"))
            doipv6 = 0;

        else if (strstr (argv[currarg], "-6"))
            doipv4 = 0;

		/* 02Feb2024, Maiko, should have added this 'else' line */
		else port = atoi (argv[currarg]);

		currarg++;

		/*
		 * This catches the port if -4 or -6 is specified, although
 		 * we have to be carefull, because if they are not specified
	 	 * and options are in place, atoi returns 0, so watch it, or
		 * else the port gets clobbered and set to ZERO, ugly ...
	 	 */
		if (argc > 2)
		{
			/* 02Feb2024, Maiko, Watch for that ZERO !!! */
        	ret = atoi (argv[currarg]);
	 ...
	 * 08Jul2023, Maiko, change from a 'for' loop to 'while' loop.
	 * 27Sep2023, Maiko, another revamp, my head screwed on backwards.
	 */
	while (currarg < argc)
	{
		tprintf ("currarg %d [%s]\n", currarg, argv[currarg]);

		/* just CR */
		if (!stricmp(argv[currarg],"cronly"))
			flags |= 0x01;

		/* 19Apr2016, Maiko (VE4KLM), the new SOCK_ENABLE_IAC flag */
        	if (!stricmp(argv[currarg],"noiac"))
			flags |= 0x02;
	 ...
	 *
	 * 22Feb2023, Maiko, Okay we can now start ipv6 and ipv4 listeners
	 *
	 * 08Jul2023, Maiko, Option now to run both listeners
	 *
	 * 27Sep2023, Maiko, Revamped argument processing
	 */
    ...
    if (!ipv6iface && doipv6)
    {
        tprintf ("not running telnet ipv6, since ipv6 iface is not defined\n");
        doipv6 = 0;
    }

	if (doipv6)
    	ret = new_start_tcpv6 (port, "MBOX Server", mbx_incom, 8192, flags);

	if (doipv4)
    	ret = new_start_tcp (port, "MBOX Server", mbx_incom, 8192, flags);
