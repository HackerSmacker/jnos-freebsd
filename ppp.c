 ...
 *
 *  Jul 96  Louis A Kotze <louisk@itc.co.za>
 *      Added IPCP Option 3 to negotiate IP Addreses.
 *
 *  Jan 2021 Maiko Langelaar (VE4KLM) maiko@pcs.mb.ca
 *      Modified to allow me to use PPP within the new interface I have
 *      written for the EA5HVK VARA HF software modem. This lets me run
 *      an IP Bridge between 2 JNOS systems over HF radio - very cool.
 */
...
#endif /* PPP */
#if defined(PPP) || defined(MBFWD)  
/*
 * FCS lookup table as generated by fcsgen.c.  Referenced by forward.c too.
 */
int16 DFAR fcstab[256] = {
...
#define SP_CHAR         0x20
#endif /* defined(PPP) || defined(MBFWD) */
#ifdef PPP  
...
/*
 * 14Jan2021, Maiko (VE4KLM), I need PPP if I am able to 
 * get VARA HF to work with any IP protocol, that's how
 * the SCS PACTOR guys do their PACTOR IP BRIDGE stuff.
 *
 * So I need to make the ppp routines work with a socket
 * connection to the VARA HF data stream. Just came to me
 * at 4 am, couldn't sleep, can't believe I am going back
 * to this 1992 era code, hopefully it works.
 *
 * Make a couple of hardware abstract layer functions ...
 *
 * 15Jan2021, Maiko (VE4KLM), Need a third function (for TX)
 *
 */

static int ppp_getchar (int type, int dev)
{
   int c;

   switch (type)
   {
      /* asynchronous device, serial port */
      case 0:
         c = get_asy (dev);
         break;

#ifdef EA5HVK_VARA	/* 29Aug2023, Maiko, VE4KLM */
      /* asynchronous device, VARA port */
      case 1:
         c = get_vara (dev);
         break;
#endif
...
static int ppp_putchar (int type, int dev, struct mbuf *bp)
{
   int c;
...
/* 15Jan2021, Maiko (VE4KLM) */
static int ppp_tx_active (int type, struct iface *ifp)
{
   int txActive = 0;
   ...
   switch (type)
   {
      ...
      case 0:
          ...
          txActive = (Asy[ifp->dev].sndq != NULLBUF);

	  break;

      /* asynchronous device, socket to data stream */
	 ...
	 */
	  ...
	  txActive = (ifp->vara->txbuf > 0);	/* give this a try instead */
#endif
          break;
   }
...
struct mbuf *bp;
{
	...
	/* 
	 * 17Apr2021, Maiko, no more hardcoded type, we need to be able to
	 * use the PPP functions in case someone still uses serial PPP.
	 */
#ifdef EA5HVK_VARA	/* 29Aug2023, Maiko, VE4KLM */
    int ppp_vara = (ifp->vara != NULL);
#else
	int ppp_vara = 0;
#endif
    ...
    *cp++ = (ph.protocol & 0x00ff);
     ...
     * being transmitted. 15Jan2021, Maiko, Modified for VARA or ASY !
     * 17Apr2021, Maiko, no more hardcoded type, we need to be able to
     * use the PPP functions in case someone still uses serial PPP.
     */

     if (ppp_tx_active (ppp_vara, ifp))
     {
/*
#ifdef UNIX
...
*/
    ...
    /* Tie off the packet */
       ...
       /*
     	...
     	* use the PPP functions in case someone still uses serial PPP.
        */
        return ppp_putchar (ppp_vara, ifp->dev, vbp);	/* 14Jan2021, Maiko (VE4KLM) */
    }
...
/* (process started by ppp_init) */
	...
	int ppp_vara = 0;
        ...
        while ((c = ppp_getchar(ppp_vara, dev)) != -1) {	/* 14Jan2021, Maiko (VE4KLM) */
