...
struct mbx *newmbx ()
{
	...
	/* 27Apr2021, Maiko (VE4KLM), We should really log any errors */

	if (MbMaxUsers && BbsUsers >= MbMaxUsers)
	{
		log (-1, "newmbx failed - %d BBS users, maximum %d",
			BbsUsers, MbMaxUsers);
	...
	{
		log (-1, "newmbx failed - unable to allocate memory");
...
struct mbx *m;
{
	...
	/* 19Feb2023,Maiko, trying to figure out peername stuff */
	//log (m->user, "peername %x %x %x %x %x %x", *tmp, *(tmp+1), *(tmp+2), *(tmp+3), *(tmp+4), *(tmp+5));
...
#endif
/*
	log (m->user, "switch SA family %x IN6 family %x IN family %x",
		sp.sa->sa_family, sp.in6->sin6_family, sp.in->sin_family);
*/
    ...
    switch(sp.sa->sa_family){
	  ...
	  if( vara ){
	    memcpy(m->call,sp.va->va_addr,AXALEN);
	  } else {
	  ...
	  if( vara ){
        ...
        /* Try to find the privileges of this user from the userfile */
            if((m->privs = userlogin(m->name,m->line,&m->path,MBXLINE,
                &anony,vara ? "varaperm" : ax_25 ? "ax25perm" : "nrperm")) == -1){
                m->privs = 0;
                free(m->path);
                m->path = NULLCHAR;
            }
		 ...
		 */
            if (!callcheck (m->name))    /* function courtesy of K2MF */
            {
                ...
                logmbox(m->user, m->name, vara?"bad VARA login":"bad AX.25 login");
        ...
        case AF_LOCAL:
#ifdef	IPV6
	/* 19Feb2023, Maiko (VE4KLM), Is it really this easy, not quite */
        case AF_INET6:
#endif
        case AF_INET:
            m->state = MBX_LOGIN;
		...
		if(vara) cp = "varaperm";
             ...
             */
                    ...
                    if( (setcall(m->call,m->name) == -1) || (!founddigit) ) {
                        m->privs &= ~(AX25_CMD|NETROM_CMD|VARA_CMD);
                    }
#else
                    m->privs &= ~(AX25_CMD|NETROM_CMD|VARA_CMD);
...
#ifdef	DONT_COMPILE
	log (s, "proc %s stk %ld siz %d auto %ld", Curproc->name,
		(long)(Curproc->stack), (int)(Curproc->stksize), (long)(&t_l));
#endif
...
#ifdef	J2KILLBYCALL
    if (m->mbox)	/* 14Sep2021, Maiko, Should make sure there is an mbox !!! */
    {
	/* 18Aug2021, Maiko (VE4KLM), Need to free these, memory leak if we don't */
	if ((m->mbox)->from)
		free ((m->mbox)->from);
	if ((m->mbox)->to)
		free ((m->mbox)->to);

    	free (m->mbox);	/* 14Sep2021, Maiko, This 'should' be inside the conditional */
    }
#endif
