...
#ifdef IPV6
#include "ipv6.h"	/* 23June2023, Maiko, This should be here ! */
#endif
...
/* 09June2023, Maiko, don't need these anymore, already at the top */
...
/* 08June2023, Maiko, Moved mailroute functions up here, then no need to prototype before hand */

#ifdef IPV6

/* cut code duplication */
static void tracelocalmail ()
{
#ifdef SMTPTRACE
   if (Smtptrace > 6)
    printf("Local mail\n");
...
/*
 * Need a function to handle both IPV4 and IPV6
 * 07June2023, Maiko (VE4KLM)
 */
void mailroute_ipvX (char *dest, int32 *destaddr, unsigned char *destaddr6)
{
	int32 a;

	*destaddr = 0;	/* important init */
	...
	if (ipv6iface)	/* 11Jun2023, Maiko, Only do this if IPV6 is enabled */
	{
		/* should I just leave local mail as IPV4 only ? not sure */

   		copyipv6addr (resolve6 (dest), destaddr6);

		if ((destaddr6[0] != 0x00) && !memcmp (destaddr6, myipv6addr, 16))
		{
			tracelocalmail();
			copyipv6addr (myipv6addr, destaddr6);
			return;
		}
	}

	if ((a = resolve(dest))!=0 && ismyaddr(a))
	{
		tracelocalmail();
		*destaddr = Ip_addr;
		return;
    ...
    if (UseMX)
	{
   		copyipv6addr (resolve6_mx (dest), destaddr6);

#ifdef SMTPTRACE
	    if (Smtptrace > 6)
       		printf("MX lookup returned = %s\n", ipv6shortform (human_readable_ipv6 (destaddr6), 0));
#endif
		if (destaddr6[0] == 0x00)	/* if fail, fall back to ipv4 */
		{
...
/* 04Dec2014, If dest addr is BAD, then why the heck should we gateway it ? */
    if(*destaddr == 0)
	{
		...
		/* 11Jun2023, Maiko, Only do this if IPV6 is enabled */
		if (ipv6iface && destaddr6[0] != 0x00)
			log (-1, "no gateway of bad address support for ipv6 yet");

		else if((*destaddr = a) == 0)
		{
            if (Gateway.ipaddr != 0)
...
#endif
	/*
	 * no point duplicating what address resolver above already told us
	 *
#ifdef SMTPTRACE
    if (Smtptrace > 6)
...
/* 09June2023, Maiko, Moved lookup functions up here, then no need to prototype before hand */

#ifdef	IPV6

/* 09Jun2023, Maiko, Need a lookup for both IPV6 and IPV4 */

static struct smtpcli *
#ifdef J2_MULTI_SMTP_SESSION
lookup_ipvX (int32 destaddr, unsigned char *destaddr6, char *prefix)
#else
lookup_ipvX (int32 destaddr, unsigned char *destaddr6)
#endif
{
    register int i;

	int gotit;	/* 11June2023, Maiko, Only if ipv6iface is enabled */
    ...
    for(i=0; i<MAXSESSIONS; i++)
	{
		...
		if (ipv6iface)	/* 11Jun2023, Maiko, Only do this if IPV6 is enabled */
		{
			gotit |= (*destaddr6 && !memcmp (cli_session[i]->ipdest6,
				destaddr6, 16) && !strcmp (cli_session[i]->prefix, prefix));
		}

		if (!gotit)
		{
        	gotit |= (destaddr && cli_session[i]->ipdest == destaddr
				&& !strcmp (cli_session[i]->prefix, prefix));
		}
/*
		if ((*destaddr6 && !memcmp (cli_session[i]->ipdest6, destaddr6, 16) && !strcmp (cli_session[i]->prefix, prefix)) ||
            (destaddr && cli_session[i]->ipdest == destaddr && !strcmp (cli_session[i]->prefix, prefix)))
*/
#else
		if (ipv6iface)	/* 11Jun2023, Maiko, Only do this if IPV6 is enabled */
		{
			gotit |= (*destaddr6 && !memcmp (cli_session[i]->ipdest6, destaddr6, 16));
		}
		if (!gotit)
			gotit |= (destaddr && cli_session[i]->ipdest == destaddr);
/*
	if ((*destaddr6 && !memcmp (cli_session[i]->ipdest6, destaddr6, 16)) ||
            (destaddr && cli_session[i]->ipdest == destaddr))
*/
#endif
		if (gotit)
 ...
 *
 * 31May2023, Maiko (VE4KLM), Attempt at IPV6 support, the
 * first 'challenge' is that smtp_fwd is passing an IPV4
 * value to us, so need to also look at newproc() call.
 */
...
#ifdef	IPV6
    /* 31May2023, Maiko (VE4KLM), Attempt at IPV6 support */
    unsigned char destaddr6[16];
    /* not used yet - unsigned char target6[16]; */
#endif
    FILE *wfile;
#ifndef UNIX
    int  qcount=0;
...
#ifdef	IPV6
	/* 08Jun2023, Maiko, New mailroute for both ipv6 and ipv4 */
	mailroute_ipvX (tmpstring, &destaddr, destaddr6);

	/* 11Jun2023, Maiko, Only do second part if IPV6 is enabled */
	if ((destaddr == 0) && (!ipv6iface || (destaddr6[0] == 0x00)))
#else
 ...
 */
#ifdef IPV6
        if ((cb = lookup_ipvX (destaddr, destaddr6, prefix)) == NULLSMTPCLI)
#else
...
#ifdef IPV6
        if ((cb = lookup_ipvX (destaddr, destaddr6)) == NULLSMTPCLI)
#else
...
#ifdef IPV6
			/*
			 * 09Jun2023, Maiko, May have to play with how we do this ?
			 * 11Jun2023, Maiko, Don't bother if ipv6iface is not defined
			 */
			if (ipv6iface && *destaddr6)
				copyipv6addr (destaddr6, cb->ipdest6);
			else
#endif
            cb->ipdest = destaddr;
 ...
 *
 * 09June2023, Maiko (VE4KLM), Attempt to support IPV6 as well ...
 */
static void
...
void *p;
{
...
#ifdef IPV6
 	/* 09June2023, Maiko (VE4KLM), Attempt to support IPV6 */
	struct j2sockaddr_in6 fsocket6;
#endif
	short family;	/* Maiko, 09June2023 */
    char *cp;
    int32 Altmx[5];
    int rcode, rcpts, goodrcpt, i, smtpbatch;
    int init = 1;
	int connected = 0;
#ifdef	WPAGES
	int smSentIdx = 0;
...
#ifdef	IPV6
	/* 11Jun2023, Maiko, Don't bother if ipv6iface is not defined */
	if (ipv6iface && (*(cb->ipdest6)))
	{
		copyipv6addr (cb->ipdest6, fsocket6.sin6_addr.s6_addr);
    	family = fsocket6.sin6_family = AF_INET6;
    	fsocket6.sin6_port = IPPORT_SMTP;
	}
	else
	{
#endif
    ...
    family = fsocket.sin_family = AF_INET;
    fsocket.sin_port = IPPORT_SMTP;
#ifdef IPV6
	}
#endif
    cb->s = j2socket(family,SOCK_STREAM,0);
    sockmode(cb->s,SOCK_ASCII);
    j2setflush(cb->s,-1); /* We'll explicitly flush before reading */
#ifdef	SGW_EXCEPTIONS
	/* 09Jun2023, Maiko, note to myself, need to put ipv6 info here */
	if (smtp_debug)
...
#ifdef IPV6
	/* 11Jun2023, Maiko, Don't bother if ipv6iface is not defined */
	if (ipv6iface && (family == AF_INET6))
    	i = j2connect(cb->s,(char*)&fsocket6,sizeof(struct j2sockaddr_in6));
	else
#endif
    	i = j2connect(cb->s,(char*)&fsocket,SOCKSIZE);
    ...
    SmtpUsers++;
 	...
 	esmtp = 0;	/* 09June2023, Maiko, Default no ESMTP */
	...
	/* 09June2023, Maiko, Check remote server header if ESMTP */
    if (strstr (cb->buf, "ESMTP"))
		esmtp = 1;
  
	/*
	 * Don't try LZW if it's us
	 * 11June2023, and don't try LZW if we're talking to ESMTP server
	 */
    if (!esmtp && Smtpclzw && (ismyaddr((int32)fsocket.sin_addr.s_addr)==NULLIF))
	{
        char cp[LINELEN];
        sendcmd(cb,"XLZW %d %d\n",Lzwbits,Lzwmode);
        usflush(cb->s);
	...
	/* 11June2023, Maiko, ESMTP does not like this 'batch mode', force off */
		if (esmtp)
			smtpbatch = 0;
		else
        	smtpbatch = Smtpbatch;
...
#else
    smtpbatch = Smtpbatch;
    // if(!smtpbatch)		/* 11Jun2023, Maiko, Yeah, can't have this */
	{
        rcode = getresp(cb,200);
        if(rcode == -1 || rcode >= 400)
			return (do_quit (cb));
		...
		/* 11Jun2023, Maiko, Batch is off already, that's good for ESMTP */
    }

	/*
	 * 11Jun2023, Maiko (VE4KLM), BUT if batch is on and we arrive here, then
	 * we have yet to read the remote server greeting, and need to do the check
	 * further down. But the way the code is written at the moment, there is no
	 * opportunity to read the greeting, since we send HELO right away, so that
	 * suggests to me we should do that now (should not do any harm), right ?
	 * Or better just do the above getresp() call irregardless of smtpbath,
	 * at this stage anyways, there is no other way to identify from start.
	 */

#endif

	if (!smtpbatch)	log (cb->s, "disabling smtp batch for this one");

/*
 * 09June2023, Maiko (VE4KLM), Lovely ... testing with EXIM remote smtp server,
 * and already running into problems, need to look at the greeting sent by the
 * remote server, and if ESMTP shows up then we need to send EHLO instead !
 */
	if (esmtp)
	{
		/* Say EHLO */
...
#ifdef	IPV6
		/*
		 * 09June2023, Maiko (VE4KLM), test time, need to talk to Bob
		 * 11Jun2023, Maiko, Don't bother if ipv6iface is not defined
		 */
		if (ipv6iface && (*(cb->ipdest6)))
            printf("Trying Connection to %s\n", ipv6shortform (human_readable_ipv6 (cb->ipdest6), 0));
		else
#endif
            printf("Trying Connection to %s\n",inet_ntoa(cb->ipdest));
		}
...
sendcmd(struct smtpcli *cb,char *fmt, ...)
{
    ...
    if(Smtptrace>1){
        printf("smtp sent: ");
	/* 09June2023, Maiko, This vprintf crashes JNOS
        vprintf(fmt,args);
		 */
    }
#endif
...
int mincode;    /* Keep reading until at least this code comes back */
{
	...
	*/
    ...
    /* Get line */
        ...
        rval = atoi(line);
/*
 * 09Jun2023, Maiko (VE4KLM), I need to save the remote server greeting, in
 * case I connect to an ESMTP server (like Bob's server I am testing with.
 *  (is it only code 220 I have to worry about ? guess we shall see)
 */
	if (rval == 220)
	{
		log (cb->s, "[%s]", line);
