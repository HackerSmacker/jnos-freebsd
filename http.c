...
int port, k;
...
#ifdef IPV6		/* 03Oct2023, Maiko (VE4KLM), Add IPV6 support */

	int doipv4, doipv6, ret = 0, currarg = 1, argthresh = 1;

 	char *dirname, drive = 'c'; /* 03Oct2023 */

	doipv4 = doipv6 = 1;	/* default both ipv4 and ipv6 listeners */

	port = IPPORT_HTTP;

	dirname = j2strdup (Httpdir);	/* defaults for rootdir and drive */

	/*
	 * 27Sep2023, Maiko (VE4KLM), support IPV6 !
	...
	tprintf ("argc %d\n", argc);
		...
		if (argc > argthresh++) /* 28Sep2023, Maiko, Oops forgot this check */
			port = atoi (argv[currarg++]);

		/* 03Oct2023, Maiko, Drive and root dir */

		if (argc > argthresh++)
			drive = argv[currarg++][0];

		if (argc > argthresh++)
			dirname = j2strdup (argv[currarg]);
	}
#else
	if(argc < 2)
		port = IPPORT_HTTP;
	...
	/* 03Oct2023, Maiko, Make sure the args are properly processed !!! */

    tprintf ("port %d drive %c dir %s\n",
     ports[k].port, ports[k].DR, ports[k].dirname);

#ifdef	IPV6	/* 03Oct2023, Maiko (VE4KLM), Adding IPV6 support */

	if (!ipv6iface && doipv6)	/* don't bother if ipv6iface is disabled */
	{
		tprintf ("not running http ipv6, since ipv6 iface is not defined\n");
		doipv6 = 0;
	}

tprintf ("ipv4 %d ipv6 %d\n", doipv4, doipv6);

	/* 27Sep2023, Maiko, Option now to run both an ipv4 and ipv6 listener */
	if (doipv6)
      ret = new_start_tcpv6 (port, "HTTP6 Server", http, 1560, 0);

	if (doipv4)
      ret = new_start_tcp (port, "HTTP Server", http, 1560, 0);
