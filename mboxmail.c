...
static void capture_sid (struct mbx *m, char *argv)
{
 	...
 	*/
#ifdef	IPV6
	/* 04Apr2023, Maiko, Need to make sure these buffers can handle IPV6 */
	char fsocket[sizeof(struct j2sockaddr_in6)];
	int i = sizeof(struct j2sockaddr_in6);
#else
	...
	*sidcopy = m->stype; strcpy (sidcopy + 1, argv);
	...
	if (j2getpeername (m->user, fsocket, &i) != -1)
	{
		 ...
		 *
		 * 04Apr2023, Maiko, Don't do this check if AF_INET6 (10) family
		 * since it truncates the IPV6 address displayed to the user, but
		 * leaves the port in there, need to clean it up at some point.
		 */
		if ((fsocket[0] == 0x10) && (tptr = strchr (psock, ':')))
		{
 ...
 */
	...
	/*
	 * 06May2023, Michael (WZ0C), Signature should not be added for (T)raffic type.
	 * The body of a Traffic message should only include the Radiogram. Preceding
	 * R-lines are okay. This will be especially an issue in cases where Traffic
	 * messages are received and parsed programmatically.
	 */
    if(!(m->sid & MBX_SID) && (m->stype != 'T')) /* not a forwarding BBS and not Traffic */
	{
...
#ifdef DEBUG_MBX_HDRS
    log (m->user, "fullfrom [%s] name [%s] to [%s] host [%s]", fullfrom, m->name, m->to, host);
#endif
...
#ifdef	J2_FILESENTMSGS
	/*
	 * 19May2022, Maiko (VE4KLM), Would be really nice to have a history
	 * of all messages a user sends from the BBS prompt, something which
	 * should have been put in years ago, and keep forgetting to do it.
	 *
	 * Originally wrote a new j2filesentmsg() function to do this, then
	 * realized it could be done using just addlist() and queuejob() !
	 *
	 * 30Jun2022, Maiko, This works, but the index TO value is not correct
	 * when you list your own messages, it is showing your own call, not who
	 * you sent it to - how does one fix it at this stage of the process ?
	 *
	 * If I am correct, the last parameter of addlist is 'aux' which (if set)
	 * forces smtpserv to reindex with that value, instead of the TO value,
	 * and I can confirm that is the case with smtpserv.c debug code, so ?
	 *
 	 * The index is supposed to be updated in smtpserv.c, but that seems
 	 * to not be happening because the 'type' var in smtpserv.c is zero,
	 * so why is that ? This would affect other index updates as well ?
	 *
	 * Running 'index ve4klm' at JNOS console fixes my listed messages,
	 * so for sure this is a case of smtpserv.c NOT updating index - so
	 * it might actually be the WriteIndex() function itself, checking,
	 * yet console rebuild fixes it, what's the difference then ???
	 *
 	 * Resolution (PENDING)
	 *
	 */
    addlist(&cclist,m->name,0, NULLCHAR);
#endif
    fseek(m->tfile,0L,0);
