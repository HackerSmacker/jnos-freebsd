...
/* 23Jun2023, Maiko */
extern void echo6_proc (unsigned char *src, \
  unsigned char *dst, struct icmpv6 *icmpv6, struct mbuf *bp);
 ...
 */
  ...
  /* 01Feb2023, Maiko, need new ntohicmpv6 functions (not done yet) */
	...
	// log (-1, "icmpv6_input - type %d", type);
/*
 ...
 */
	...
	/* 07Feb2023, Maiko, I had ttl set to 0, setting to 64, BUT, why did it not grab MAXTTL in _send() ? */
        ...
        case ICMPV6_ECHO_REPLY:   /* Echo Reply */
            icmpInEchoReps++;
	/*
	 * 06Apr2023, Maiko, need this now if we are to do outgoing pings
	 *
	 * 06Feb2023, Maiko (VE4KLM), actually this is just a notification
	 * function deal with it later ...
	 */
            echo6_proc (ipv6->source, ipv6->dest,&icmpv6,bp);
 ...
 *
 * 05Apr2023, Maiko, Oops next_header == ICMPV6_PTCL, not ICMP_PTCL
 */
...
*/
    ...
    if(uchar(ipv6->next_header) == ICMPV6_PTCL)
	{
