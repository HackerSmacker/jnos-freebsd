...
struct mbuf *bp;
{
	...
	int nomark = 0;
#endif
	int aprsMICEdata = 0;	/* 04Sep2023, Maiko */
	...
	/*
	 * 04Sep2023, Maiko, VE4KLM, I do NOT want to log APRS mic'e data
	 * even if APRS is not defined or in use on this JNOS system. I'm
	 * seeing the ax25 heard destination list get uncontrollably huge
	 * if there is regular mic'e data coming in, it's terrible, so to
	 * save duplication of code, let's use this section to set a flag
	 * as to whether logdest should be called or not further below.
	 *
	...
	*/
	...
	/*	log (-1, "idest [%s]", pax25 (itmp, idest)); */
		...
		/* 08Nov2002, Maiko, My first attempt at supporting WIDEn-N digi */
		  ...
		  aprslog( -1, "  Checking whether to digipeat" );
		  {
		    struct mbuf *tbp;
		    char buf[300];
		    int buflen;
		    ...
		    /* 28Sep2023, WZ0C, add buffer and buflen 
		     * 02Oct2023, Maiko, add hwaddr for having multiple APRS interfaces
		     */
		    digiaddr = aprs_digi (&hdr, idest, &nomark, buf, buflen, iface->hwaddr );
    ...
    (!Nr_iface || !addreq(isrc,Nr_iface->hwaddr))) {
        ...
        if(!addreq(isrc,iface->hwaddr)
			...
			if (!aprsMICEdata)			/* 04Sep2023, Maiko */
            	logdest(iface,idest);
        }
