...
#include "icmpv6.h"	/* 15May2023, Maiko, for checksum function */
#endif

#define NLP nos_log_peerless  /* 17Apr2023 (yeah even for tcp this is handy) */
static char *nobodylistening = "no TCP (%d) listener";	/* 17Apr2023 */
...
int process_tcb (struct iface *iface, struct connection *conn, struct mbuf *bp, struct tcp *seg, int16 *length, char *tos, char congest)
{
	...
	/*
	 * 12May2023, Maiko (VE4KLM), Terrible mistake, there has been NO
	 * tcp access functionality since 13Feb2023, when I commented it
	 * out (without realizing it affects BOTH ipv4 and ipv6), a bad
	 * move on my part, so make sure this works at least for ipv4.
	 *
	 * Note : still no tcp access for ipv6 - just use linux side for now
	 *
	...
	{
        if(tcp_check(TCPaccess,iconn.remote.address,seg->dest) != 0 )
#else
        if(tcp_check(TCPaccess,ip->source,seg->dest) != 0 )
#endif
		{
	/* 12May2023, Maiko (VE4KLM), Now tracking tcp access denied packets */
		extern unsigned int tcpaccessdropcnt;	/* in tcpcmd.c */
		tcpaccessdropcnt++;
	...
	log (-1, "figure out check for tcp_listen for ipv6 still");
        ...
        if ((tcb = lookup_tcb (conn)) == NULLTCB)
		{
			...
			{
#ifdef	IPV6
				if (iconn.ipver == IPV6VERSION)
				{
        			/* 17Apr2023, Maiko, All this trouble just to do peerless logging */
        			struct j2sockaddr_in6 fsock6;
        			copyipv6addr (iconn.remotev6.address, fsock6.sin6_addr.s6_addr);
        			fsock6.sin6_port = iconn.remotev6.port;
        			fsock6.sin6_family = AF_INET6;  /* 17Apr2023, Maiko (very important) */

					/* 17Apr2023, Maiko, log this actually, useful I think (use peerless code) */
					NLP (&fsock6, nobodylistening, iconn.localv6.port);
				}
				else
				{
#endif
					...
					fsock.sin_addr.s_addr = iconn.remote.address;
        			fsock.sin_port = iconn.remote.port;
        			fsock.sin_family = AF_INET;  /* 17Apr2023, Maiko (very important) */

					/* 17Apr2023, Maiko, log this actually, useful I think (use peerless code) */
					NLP (&fsock, nobodylistening, iconn.local.port);
#ifdef IPV6
				}
#endif
			}
...
int rxbroadcast;    	/* Incoming broadcast - discard if true */
{
 ...
 char dummy_was_tos;	/* 14Feb2023, Maiko (VE4KLM), traffic class ??? */
    ...
    length = ipv6->payload_len;	/* 15Feb2023, Maiko, does not include header */

	/*
	 * 04May2023, Maiko (VE4KLM), FINALLY doing proper checksum on
	 *  incoming packets, had this commented out for a long time !
	 *   (damn, still not working)
	 *
	 * 15May2023, Maiko, you HAVE to prototype Mikey, that's why, remember
	 *  the length is int16, probably compiler being picky, as it should.
	 */
    if (icmpv6cksum (ipv6, bp, length) != 0)
    {
	tprintf ("tcp checksum error \n");
...
#ifdef	IPV6
/* 05Aug2023, Maiko, An oversite, missing callbacks, misfired starts */
	conn.ipver = IPVERSION;
#endif
...
struct tcb *tcb;
{
    ...
    tcb->iss = (int32)((msclock() % INT_MAX) << 12);
