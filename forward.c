    ...
    if(fwdinit(f.m) != -1) {
                ...
                if(read_header(idx,&hdr) != -1)
		{
                    for(i=1; i<=f.m->nmsgs; i++)
			{
						...
						j2pause(200);
#endif
					/* 02May2023, Maiko, Not sure what I was smoking at the time */
                        pwait(NULL);
...
void *p;
{
    struct mbx *m;
#ifdef IPV6
    /* 06May2023, Maiko, Oops, forgot to up the size of the peername buffer */
    char sock[sizeof(struct j2sockaddr_in6)];
#else
    ...
    switch(*argv[0])
    {
        ...
        */
        case 'v':
	     ...
	     sp.va->iface_index = if_indexbyname(argv[1]);  
	     setcall(sp.va->va_addr,argv[2]); /* the remote callsign */

#if 0
	     /* not supported yet */
	     if( argc > 3 )  /* process digipeater string; do auto-ax25-route-add */
	       if(connect_filt(argc,argv,sp.va->va_addr,if_lookup(argv[1])) == 0)
		 return -1;
#endif

	     if((m->user = j2socket(AF_VARA,SOCK_STREAM,0)) == -1) {
	       if(Mtrace)
		 j2tputs("fwd: Unable to open vara socket\n");
	       return -1;
	     }
	     len = sizeof(*sp.va);
	     m->family = AF_VARA; /*So the user list will be correct! - WZ0C */
	     remote = argv[2];
             break;
#endif
        case 't':
