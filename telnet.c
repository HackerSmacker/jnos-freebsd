...
#include <string.h>	/* 06Mar2023, Maiko, needed for strchr() prototype */
...
void *p;
{
	...
	int16 port; /* to miminize code */
	int doipv6, doipv4, ipvarg = 0;	/* 18May2023, Maiko, for -4 or -6 options */
#endif
	int family;	/* 06Mar2023, Maiko, for both v4 and v6 */
    char *name, firstch;
	...
	if (argc > 1)	/* important, since bbs command is 'different' */
	{
		if (strstr (argv[1], "-4"))
		{
			doipv6 = 0;
    ...
    name = argv[1+ipvarg];     /* This is not valid for 'bbs' command ! */
    ...
    // log (-1, "session name [%s]", sp->name);

    if(argc < ((auth ? 5 : 3) + ipvarg)) {  /* is the port defaulted? */
#ifdef LZW
        if(lzw)
            port = IPPORT_XCONVERS;
        else
#endif
#ifdef TTYLINKSESSION
        if(split)
...
//          fsocket.sin_port = IPPORT_NODE;
	...
	if (doipv6)
	{
		if (ipv6iface)
		{
			tprintf ("Resolving %s (ipv6) ... ", sp->name);
    		tflush();
		}
		 ...
		 */
		copyipv6addr (resolve6 (sp->name), fsocket6.sin6_addr.s6_addr);
	}

    // careful, you have to break out doipv6 IF for the auto ipv4 to work !

	if (doipv6 && (fsocket6.sin6_addr.s6_addr[0] != 0x00))		/* if resolve fails, goto ipv4 */
	{
   		family = fsocket6.sin6_family = AF_INET6;
		fsocket6.sin6_port = port;
	}
...
#endif
	}	/* from around# BBSSESSION */

	// log (-1, "family %d", family);

    if((sp->s = j2socket(family ,SOCK_STREAM,0)) == -1){
        j2tputs(Nosock);
        keywait(NULLCHAR,1);
        freesession(sp);
        return 1;
...
#endif
#ifdef MD5AUTHENTICATE
    if (auth && argc > (3+ipvarg)) {  /* etelnet dest [port] loginid passwd */
        sp->ufile = j2strdup(argv[argc-2+ipvarg]);  /* Reuse the upload filename ptr briefly */
        sp->rfile = j2strdup(argv[argc-1+ipvarg]);  /* Ditto, for record filename ptr */
	...
	/* 06Mar2023, Maiko, tel_connect already takes a 'generic' socket buffer, nice ! */
	if (family == AF_INET)
    	return tel_connect(sp,(char *)&fsocket,SOCKSIZE);
	else
    	return tel_connect(sp,(char *)&fsocket6,sizeof(fsocket6));
}
...
#ifdef IPV6 
	/* 09Mar2023, Maiko, Get the proper information displayed to user */
	if (strchr (sp->name, ':'))
		printf("Trying ...\n");
	else
#endif
